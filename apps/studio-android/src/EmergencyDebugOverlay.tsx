...
function getWebViewPipelineStatus(snap: any) {
  if (!snap)
    return { status: 'UNKNOWN', color: 'rgba(255,255,255,0.4)', desc: 'No snapshot selected' };
  ...
  const opacityVal =
    hubAudit?.opacity !== undefined ? hubAudit.opacity : snap.elements?.['hub-root']?.opacity;
  const opacity = parseFloat(opacityVal || '0');

  const domSaysVisible =
    hubMounted && display !== 'none' && visibility !== 'hidden' && opacity > 0.05;
...
function EmergencyDebugOverlayInner() {
      ...
      if (panel && !health.panelTopmost) {
        try {
          ...
          while (curr && curr !== document.body && curr !== document.documentElement) {
            ...
            elementsAbove.push(
              `${curr.tagName.toLowerCase()}${curr.id ? '#' + curr.id : ''}${curr.className ? '.' + Array.from(curr.classList).join('.') : ''} (z-index: ${style.zIndex}, pointer-events: ${style.pointerEvents}, opacity: ${style.opacity})`
            );
  ...
  const getVisualBlockers = () => {
    ...
    for (const [key, pt] of Object.entries(coords)) {
      try {
        ...
        const isHub =
          !!el?.closest?.('[data-livex-hub-root="true"]') || !!el?.closest?.('.app-main-layout');
  ...
  const getDiagnosticsPayload = () => {
    ...
    const hubShellFound =
      statePayload.hubShellFound !== undefined
        ? statePayload.hubShellFound
        : !!document.querySelector('.app-container.app-mode-hub .app-main-layout');
    const hubRootFound =
      statePayload.hubRootFound !== undefined
        ? statePayload.hubRootFound
        : !!document.querySelector('[data-livex-hub-root="true"]');
    const hubContentFound =
      statePayload.hubContentFound !== undefined
        ? statePayload.hubContentFound
        : !!document.querySelector('[data-livex-hub-content="true"]') ||
          !!document.querySelector('.gb-wrap');
    const hubNavFound =
      statePayload.hubNavFound !== undefined
        ? statePayload.hubNavFound
        : !!document.querySelector('nav.glass-nav');
    const hubActuallyPainted =
      statePayload.hubActuallyPainted !== undefined
        ? statePayload.hubActuallyPainted
        : hubRootFound || hubContentFound || hubNavFound;
    ...
    const suspenseActive = !!document.querySelector(
      '.smart-loading, .fallback-skeleton, .studio-accent-loader, .studio-shimmer, [class*="skeleton"]'
    );
    const errorBoundaryActive = !!document.querySelector(
      '.error-boundary, [class*="error-boundary"]'
    );

    const trace = getNavigationTrace();
    const lastAction =
      statePayload.lastNavigationAction || (trace.length > 0 ? trace[trace.length - 1] : null);
  ...
  const generateSimplifiedDOMTree = () => {
    ...
    const walk = (el: Element, depth: number) => {
      ...
      if (
        el.id === 'livex-emergency-panel' ||
        el.closest('#livex-emergency-panel') ||
        el.id === 'livex-panic-menu' ||
        el.closest('#livex-panic-menu')
      ) {
        return;
      }
...
Last Navigation Action: ${lastNav ? `${lastNav.fromApp} -> ${lastNav.toApp}` : 'none'}`;
  ...
  const detectVisualObstruction = () => {
    ...
    const topmostDesc = topmost
      ? `${topmost.tagName.toLowerCase()}${topmost.id ? '#' + topmost.id : ''}${topmost.className ? '.' + topmost.className.split(' ').join('.') : ''}`
      : 'none';
  ...
  const auditReactRender = () => {
    ...
    const chordexMounted =
      !!document.querySelector('.app-sub-app-container') ||
      !!document.querySelector('[class*="chordex"]');
    const suspenseActive = !!document.querySelector(
      '.smart-loading, .fallback-skeleton, .studio-accent-loader, .studio-shimmer, [class*="skeleton"]'
    );
    const errorBoundaryActive = !!document.querySelector(
  ...
  const getPaintDiagnostics = () => {
    ...
    try {
      ...
      document.querySelectorAll('*').forEach((el) => {
        ...
        const htmlEl = el as HTMLElement;
        if (
          htmlEl.id === 'livex-emergency-overlay-root' ||
          htmlEl.closest('#livex-emergency-overlay-root')
        ) {
          return;
        }
        const style = window.getComputedStyle(htmlEl);
  ...
  const copyFullForensicsReport = () => {
    const diag = getDiagnosticsPayload();
    const classification = classifyBlackScreen(diag);
    const elementsStackCenter = document.elementsFromPoint(
      window.innerWidth / 2,
        ...
        stack
          .map(
            (
              el,
              idx
            ) => `  [${idx}] ${el.tag}${el.id ? '#' + el.id : ''}${el.className ? '.' + el.className.split(' ').join('.') : ''}
...
${
  overlays
    .map(
      (o, idx) => `${idx + 1}. Selector: ${o.selector}
   z-index:        ${o.zIndex}
   opacity:        ${o.opacity}
...
`;
  ...
  const renderForensicsTab = () => {
    ...
    const formatStackList = (stack: any[]) => {
      if (stack.length === 0)
        return (
          <div style={{ color: 'rgba(255,255,255,0.4)', paddingLeft: '8px' }}>Empty stack</div>
        );
      return stack.map((el, idx) => (
        ...
        >
          ...
          <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '9px', marginTop: '2px' }}>
            pos: {el.position} | z: {el.zIndex} | op: {el.opacity} | vis: {el.visibility} | disp:{' '}
            {el.display} | pe: {el.pointerEvents}
          </div>
        </div>
      ));
    };
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
          <button
            onClick={copyFullForensicsReport}
            style={{
              flex: 1,
              padding: '8px',
        ...
        <div style={cardStyle}>
          ...
          <div style={rowStyle}>
            ...
            <span style={valStyle(obstruction.hubExistsInDOM)}>
              {String(obstruction.hubExistsInDOM)}
            </span>
          </div>
          <div style={rowStyle}>
            <span style={labelStyle}>Hub Under Center Stack:</span>
            <span style={valStyle(obstruction.hubExistsUnderCenter)}>
              {String(obstruction.hubExistsUnderCenter)}
            </span>
          </div>
          <div style={rowStyle}>
            <span style={labelStyle}>Hub Is Covered:</span>
            <span style={valStyle(!obstruction.hubIsCovered)}>
              {String(obstruction.hubIsCovered)}
            </span>
          </div>
          ...
          <div style={rowStyle}>
            ...
            <span style={valStyle(!obstruction.hasZeroPaintArea)}>
              {String(obstruction.hasZeroPaintArea)}
            </span>
          </div>
          <div style={rowStyle}>
            <span style={labelStyle}>Hub Zero Bounds:</span>
            <span style={valStyle(!obstruction.hasZeroBounds)}>
              {String(obstruction.hasZeroBounds)}
            </span>
          </div>
          <div style={rowStyle}>
            <span style={labelStyle}>Hub Bounds:</span>
            <span style={valStyle()}>
              {obstruction.hubBounds.width}x{obstruction.hubBounds.height}
            </span>
          </div>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={rowStyle}>
            ...
            <span style={valStyle(undefined, '#fbbf24')}>
              {webViewDiag.documentVisibilityState}
            </span>
          </div>
          <div style={rowStyle}>
...
page:  (${webViewDiag.visualViewport.pageLeft}, ${webViewDiag.visualViewport.pageTop})`}
        ...
        <div style={cardStyle}>
          ...
          <div style={rowStyle}>
            ...
            <span style={valStyle(!paintDiag.allBlackFullscreenElementExists)}>
              {paintDiag.allBlackFullscreenElementExists
                ? `DETECTED (${paintDiag.blackOverlaySelector})`
                : 'none'}
            </span>
          </div>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={rowStyle}>
            ...
            <span style={valStyle(reactAudit.rootMounted)}>
              {reactAudit.rootMounted ? 'mounted' : 'empty'}
            </span>
          </div>
          <div style={rowStyle}>
          ...
          <div style={rowStyle}>
            ...
            <span style={valStyle(reactAudit.chordexMounted)}>
              {String(reactAudit.chordexMounted)}
            </span>
          </div>
          <div style={rowStyle}>
            <span style={labelStyle}>Suspense Active (Loader):</span>
            <span style={valStyle(!reactAudit.suspenseActive)}>
              {String(reactAudit.suspenseActive)}
            </span>
          </div>
          <div style={rowStyle}>
            <span style={labelStyle}>ErrorBoundary Active:</span>
            <span style={valStyle(!reactAudit.errorBoundaryActive)}>
              {String(reactAudit.errorBoundaryActive)}
            </span>
          </div>
          <div
            style={{
            ...
            >
              {reactAudit.reactComponentNames.length > 0 ? (
                reactAudit.reactComponentNames.map((name, i) => (
                  <div
                    key={i}
                    style={{ color: '#c084fc', fontSize: '9px', fontFamily: 'monospace' }}
                  >
                    &lt;{name} /&gt;
                  </div>
                ))
                ...
                <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '9px' }}>
                  No React components resolved from fiber root.
                </div>
              )}
            </div>
          </div>
            ...
            {overlays.length > 0 ? (
              overlays.map((o, i) => (
                ...
                >
                  ...
                  <div
                    style={{ color: 'rgba(255,255,255,0.5)', fontSize: '8.5px', marginTop: '2px' }}
                  >
              ...
              <div
                style={{ color: 'rgba(255,255,255,0.4)', textAlign: 'center', fontSize: '10px' }}
              >
                No overlays covering &gt;= 90% of screen found.
              </div>
            )}
          </div>
        </div>
  ...
  useEffect(() => {
    const checkAndAutoCapture = () => {
      ...
      if (hubRootMissing || isScreenBlank) {
        ...
        const path = diagnostics.navigationTrace
          .map((t: any) => `${t.fromApp}->${t.toApp}`)
          .join(' | ');
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={rowStyle}>
            ...
            <span style={valStyle(undefined, '#f59e0b')}>
              {String(useSettingsStore.getState().settings.appMode === 'hub' ? 'N/A' : 'derived')}
            </span>
          </div>
          <div style={rowStyle}>
        ...
        <div style={cardStyle}>
          ...
          <div style={rowStyle}>
            ...
            <span
              style={valStyle(
                undefined,
                classification === 'DIAGNOSTIC_SELECTOR_FALSE_POSITIVE' ||
                  classification === 'SIMULATED_BLACK_LAYER_ACTIVE'
                  ? '#fb923c'
                  : classification === 'UNKNOWN_BLACK_SCREEN'
                    ? '#9ca3af'
                    : '#f43f5e'
              )}
            >
            ...
            <>
              <div style={rowStyle}>
                ...
                <span style={valStyle(undefined, '#38bdf8')}>
                  {topmostCenterEl.tag}
                  {topmostCenterEl.id ? '#' + topmostCenterEl.id : ''}
                </span>
              </div>
              <div style={rowStyle}>
                ...
                <div style={rowValWrapStyle}>
                  <span
                    style={{ ...valStyle(undefined), wordBreak: 'break-all', textAlign: 'right' }}
                  >
          ...
          {lastNav ? (
            <>
              ...
              <div style={rowStyle}>
                ...
                <span style={valStyle()}>
                  {lastNav.transitionDuration !== undefined
                    ? `${lastNav.transitionDuration}ms`
                    : 'unknown'}
                </span>
              </div>
              <div style={rowStyle}>
                <span style={labelStyle}>lock state:</span>
                <span style={valStyle(!lastNav.transitionLockState)}>
                  {String(lastNav.transitionLockState)}
                </span>
              </div>
              <div style={rowStyle}>
                <span style={labelStyle}>recoveredViaFailsafe:</span>
                <span
                  style={valStyle(undefined, lastNav.recoveredViaFailsafe ? '#f59e0b' : '#10b981')}
                >
                  {String(!!lastNav.recoveredViaFailsafe)}
                </span>
              </div>
            </>
          ) : (
  ...
  const renderDiffRow = (label: string, beforeVal: any, afterVal: any, keyVal?: string) => {
    ...
    return (
      <tr
        key={keyVal || label}
        style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', background: rowBg }}
      >
        <td
          style={{
            padding: '4px 8px',
            color: isDiff ? '#f59e0b' : 'rgba(255,255,255,0.7)',
            fontWeight: isDiff ? 'bold' : 'normal',
          }}
        >
          {label}
        </td>
        <td style={{ padding: '4px 8px', color: '#fff', wordBreak: 'break-all' }}>
          {String(beforeVal)}
        </td>
        <td
          style={{
            padding: '4px 8px',
            color: isDiff ? '#34d399' : '#fff',
            fontWeight: isDiff ? 'bold' : 'normal',
            wordBreak: 'break-all',
          }}
        >
      ...
      if (timeline && timeline.snapshots) {
        ...
        for (const key of checkpointKeys) {
          ...
          if (snap) {
            const statusObj = getWebViewPipelineStatus(snap);
            if (
              statusObj.status === 'ROOT_APP_TREE_MISSING' ||
              statusObj.status === 'HUB_DOM_NOT_MOUNTED' ||
              statusObj.status === 'COMPOSITOR_FREEZE'
            ) {
...
Total Checkpoints: ${timeline?.snapshots ? Object.keys(timeline.snapshots).length : 0}`;
    ...
    const copySymbolicatedReactErrorReport = () => {
      const report =
        ...
        'No symbolicated React error report found.';
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        >
          <div>
            ...
            <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '10px', marginTop: '2px' }}>
              Attempted: {new Date(timestamp).toLocaleDateString()}{' '}
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <span
              style={{ fontSize: '14px', fontWeight: 'bold', color: '#60a5fa', marginTop: '2px' }}
            >
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <span
              style={{
                ...
                fontWeight: 'bold',
                color:
                  localStorage.getItem('studio_rootapp_last_error_suppressed') === 'true'
                    ? '#34d399'
                    : '#f87171',
                marginTop: '4px',
              }}
          ...
          >
            ...
            <span
              style={{ fontSize: '11px', fontWeight: 'semibold', color: '#fff', marginTop: '2px' }}
            >
              {localStorage.getItem('studio_rootapp_last_error_duration')
                ? `${localStorage.getItem('studio_rootapp_last_error_duration')}ms`
                : 'N/A'}
            </span>
          </div>
        </div>
          ...
          {checkpointKeys.map((key, idx) => {
            ...
            if (!snap) {
              return (
                <div
                  key={key}
                  style={{ display: 'flex', gap: '12px', opacity: 0.5, position: 'relative' }}
                >
                  <div
                    style={{
                      position: 'absolute',
                  ...
                  />
                  <div>
                    <span style={{ fontWeight: 'bold', color: 'rgba(255,255,255,0.4)' }}>
                      {key}
                    </span>
                    <span
                      style={{
            ...
            const statusColor = isBlack
              ? '#f43f5e'
              : paint?.paintState === 'painted'
                ? '#10b981'
                : '#f59e0b';
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div
                          style={{
                            fontWeight: 'bold',
                            fontSize: '10.5px',
                            color: snap.transitionActive ? '#f59e0b' : '#10b981',
                          }}
                        >
                      ...
                      <div style={{ flex: 1.5 }}>
                        ...
                        <div
                          style={{
                            fontWeight: 'bold',
                            fontSize: '10.5px',
                            color: snap.hubDomState?.mounted ? '#10b981' : '#f43f5e',
                          }}
                        >
                          {snap.hubDomState?.mounted
                            ? `Mounted (${snap.hubDomState.elementCount} nodes)`
                            : 'Not Mounted'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                      <div>
                        ...
                        <div style={{ fontWeight: 'bold', color: statusColor }}>
                          {paint ? String(paint.paintState).toUpperCase() : 'UNKNOWN'}{' '}
                          {paint ? `(${paint.blackPercent}% black)` : ''}
                        </div>
                      </div>
                      <div>
                        <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '9px' }}>
                        ...
                        <div>
                          {snap.webViewMetrics
                            ? `${snap.webViewMetrics.layerCount} estimated layers`
                            : 'N/A'}
                        </div>
                      </div>
                    </div>
  ...
  const renderNavForensicsTab = () => {
    ...
    // Timeline snap setup
    const availableSnaps =
      activeCapture && activeCapture.snapshots
        ? Object.keys(activeCapture.snapshots).sort((a, b) => {
    ...
    const currentLeftKey = availableSnaps.includes(leftSnapKey)
      ? leftSnapKey
      : availableSnaps[0] || 'LEAVING_CHORDEX';
    const currentRightKey = availableSnaps.includes(rightSnapKey)
      ? rightSnapKey
      : availableSnaps[availableSnaps.length - 1] || 'T+2000ms';
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        ...
        <div style={statsCardStyle}>
            ...
            {lastSuccessfulForensic ? (
              <div>
                <div style={{ fontWeight: 'bold' }}>
                  {formatTime(lastSuccessfulForensic.timestamp)}
                </div>
                <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '9px', marginTop: '2px' }}>
                  App Mode: {beforeSnapSuccess?.appMode || 'chords'} →{' '}
                  {afterSnapSuccess?.appMode || 'hub'}
                </div>
              </div>
            ) : (
              <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '10px' }}>
                No successful return captured yet
              </div>
            )}
          </div>
          ...
          <div style={{ ...halfCardStyle, borderLeft: '3px solid #ef4444' }}>
            ...
            {lastFailedForensic ? (
              <div>
                <div style={{ fontWeight: 'bold', color: '#ff8a8a' }}>
                  {formatTime(lastFailedForensic.timestamp)}
                </div>
                <div
                  style={{
                    color: '#f43f5e',
                ...
                >
                  Reason: {lastFailedForensic.reason || 'Blocked'}
                </div>
              </div>
            ) : (
              <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '10px' }}>
                No failed return captured yet
              </div>
            )}
          </div>
        </div>
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span
                  style={{
                    fontWeight: 'bold',
                    color: livePaintVerify.domExists ? '#10b981' : '#f43f5e',
                  }}
                >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span
                  style={{
                    fontWeight: 'bold',
                    color: livePaintVerify.paintState === 'painted' ? '#10b981' : '#f43f5e',
                  }}
                >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span
                  style={{
                    fontWeight: 'bold',
                    color: livePaintVerify.blackPercent > 98 ? '#f43f5e' : '#10b981',
                  }}
                >
              ...
              {livePaintVerify.thumbnail && (
                ...
                >
                  <span style={{ color: 'rgba(255, 255, 255, 0.5)', alignSelf: 'flex-start' }}>
                    Captured Screenshot:
                  </span>
                  <div
                    style={{
                  ...
                  >
                    <img
                      src={livePaintVerify.thumbnail}
                      style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                      alt="Live Paint Verification"
                    />
                  </div>
                </div>
              )}
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          {entry.paintData?.paintState || 'Unknown'},{' '}
                          {entry.paintData?.blackPercent !== undefined
                            ? `${entry.paintData.blackPercent}% black`
                            : ''}
                          )
                        </span>
                      </>
            ...
            <div>
              ...
              <div
                style={{
                  ...
                  padding: '10px',
                  borderRadius: '8px',
                  border: `1px solid ${pipelineStatus.color}`,
                  background: `${pipelineStatus.color}15`,
                }}
              >
                <div
                  style={{
              ...
              >
                <div>
                  ...
                  <span
                    style={{
                      color: activeCapture.result === 'success' ? '#10b981' : '#ef4444',
                      fontWeight: 'bold',
                    }}
                  >
                    {activeCapture.result?.toUpperCase()}
                  </span>
                  {activeCapture.reason && (
                    <span style={{ color: 'rgba(255,255,255,0.5)', marginLeft: '6px' }}>
                      ({activeCapture.reason})
                    </span>
                  )}
                </div>
                <div style={{ color: 'rgba(255,255,255,0.4)' }}>ID: {activeCapture.id}</div>
              </div>
              ...
              {activeCapture.snapshots && availableSnaps.length > 0 && (
                ...
                >
                  <span style={{ fontSize: '9.5px', color: 'rgba(255,255,255,0.6)' }}>
                    Compare snapshots:
                  </span>
                  <select
                  ...
                  >
                    {availableSnaps.map((key) => (
                      <option key={`right-${key}`} value={key}>
                        {key}
                      </option>
                    ))}
                  </select>
              ...
              >
                ...
                >
                  <thead>
                    ...
                    >
                      <th style={{ padding: '6px 8px', color: 'rgba(255,255,255,0.6)' }}>
                        Selector / Property
                      </th>
                      <th style={{ padding: '6px 8px', color: 'rgba(255,255,255,0.6)' }}>
                        LEFT ({activeCapture.snapshots ? currentLeftKey : 'BEFORE'})
                      </th>
                      <th style={{ padding: '6px 8px', color: 'rgba(255,255,255,0.6)' }}>
                        RIGHT ({activeCapture.snapshots ? currentRightKey : 'AFTER'})
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {renderDiffRow(
                      'Active SubApp',
                      leftSnapshot?.activeSubApp,
                      rightSnapshot?.activeSubApp
                    )}
                    {renderDiffRow('Stable Key', leftSnapshot?.stableKey, rightSnapshot?.stableKey)}
                    {renderDiffRow(
                      'Transition Active',
                      leftSnapshot?.transitionActive,
                      rightSnapshot?.transitionActive
                    )}
                    ...
                    {renderDiffRow(
                      'Viewport Size',
                      `${leftSnapshot?.viewport?.innerWidth}x${leftSnapshot?.viewport?.innerHeight}`,
                      `${rightSnapshot?.viewport?.innerWidth}x${rightSnapshot?.viewport?.innerHeight}`
                    )}
                    {renderDiffRow(
                      'Visual Viewport',
                      leftSnapshot?.viewport?.visualViewport
                        ? `${leftSnapshot.viewport.visualViewport.width}x${leftSnapshot.viewport.visualViewport.height} (scale: ${leftSnapshot.viewport.visualViewport.scale})`
                        : 'N/A',
                      rightSnapshot?.viewport?.visualViewport
                        ? `${rightSnapshot.viewport.visualViewport.width}x${rightSnapshot.viewport.visualViewport.height} (scale: ${rightSnapshot.viewport.visualViewport.scale})`
                        : 'N/A'
                    )}
                    {renderDiffRow(
                      'Device Pixel Ratio',
                      leftSnapshot?.viewport?.dpr,
                      rightSnapshot?.viewport?.dpr
                    )}
                    {renderDiffRow(
                      'Orientation',
                      leftSnapshot?.viewport?.orientation?.type,
                      rightSnapshot?.viewport?.orientation?.type
                    )}
                    ...
                    >
                      ...
                      >
                        WEBVIEW RENDER AUDIT
                      </td>
                    </tr>
                    {renderDiffRow(
                      'GPU Compositor Layer Count',
                      leftSnapshot?.renderAudit?.layerCount,
                      rightSnapshot?.renderAudit?.layerCount
                    )}
                    ...
                    >
                      <td
                        colSpan={3}
                        style={{ padding: '4px 8px', fontWeight: 'bold', color: '#38bdf8' }}
                      >
                        Root Container (#root)
                      </td>
                    </tr>
                    {renderDiffRow(
                      '  ↳ Rect Bounds',
                      leftSnapshot?.renderAudit?.root?.rect
                        ? `l:${leftSnapshot.renderAudit.root.rect.left} t:${leftSnapshot.renderAudit.root.rect.top} w:${leftSnapshot.renderAudit.root.rect.width} h:${leftSnapshot.renderAudit.root.rect.height}`
                        : 'N/A',
                      rightSnapshot?.renderAudit?.root?.rect
                        ? `l:${rightSnapshot.renderAudit.root.rect.left} t:${rightSnapshot.renderAudit.root.rect.top} w:${rightSnapshot.renderAudit.root.rect.width} h:${rightSnapshot.renderAudit.root.rect.height}`
                        : 'N/A'
                    )}
                    {renderDiffRow(
                      '  ↳ display',
                      leftSnapshot?.renderAudit?.root?.display,
                      rightSnapshot?.renderAudit?.root?.display
                    )}
                    {renderDiffRow(
                      '  ↳ visibility',
                      leftSnapshot?.renderAudit?.root?.visibility,
                      rightSnapshot?.renderAudit?.root?.visibility
                    )}
                    {renderDiffRow(
                      '  ↳ opacity',
                      leftSnapshot?.renderAudit?.root?.opacity,
                      rightSnapshot?.renderAudit?.root?.opacity
                    )}
                    {renderDiffRow(
                      '  ↳ transform',
                      leftSnapshot?.renderAudit?.root?.transform,
                      rightSnapshot?.renderAudit?.root?.transform
                    )}
                    {renderDiffRow(
                      '  ↳ filter',
                      leftSnapshot?.renderAudit?.root?.filter,
                      rightSnapshot?.renderAudit?.root?.filter
                    )}
                    {renderDiffRow(
                      '  ↳ contain',
                      leftSnapshot?.renderAudit?.root?.contain,
                      rightSnapshot?.renderAudit?.root?.contain
                    )}
                    {renderDiffRow(
                      '  ↳ isolation',
                      leftSnapshot?.renderAudit?.root?.isolation,
                      rightSnapshot?.renderAudit?.root?.isolation
                    )}
                    {renderDiffRow(
                      '  ↳ overflow',
                      leftSnapshot?.renderAudit?.root?.overflow,
                      rightSnapshot?.renderAudit?.root?.overflow
                    )}
                    ...
                    >
                      <td
                        ...
                        style={{ padding: '4px 8px', fontWeight: 'bold', color: '#38bdf8' }}
                      >
                        Hub Container ([data-livex-hub-root])
                      </td>
                    </tr>
                    {renderDiffRow(
                      '  ↳ Rect Bounds',
                      leftSnapshot?.renderAudit?.hub?.rect
                        ? `l:${leftSnapshot.renderAudit.hub.rect.left} t:${leftSnapshot.renderAudit.hub.rect.top} w:${leftSnapshot.renderAudit.hub.rect.width} h:${leftSnapshot.renderAudit.hub.rect.height}`
                        : 'N/A',
                      rightSnapshot?.renderAudit?.hub?.rect
                        ? `l:${rightSnapshot.renderAudit.hub.rect.left} t:${rightSnapshot.renderAudit.hub.rect.top} w:${rightSnapshot.renderAudit.hub.rect.width} h:${rightSnapshot.renderAudit.hub.rect.height}`
                        : 'N/A'
                    )}
                    {renderDiffRow(
                      '  ↳ display',
                      leftSnapshot?.renderAudit?.hub?.display,
                      rightSnapshot?.renderAudit?.hub?.display
                    )}
                    {renderDiffRow(
                      '  ↳ visibility',
                      leftSnapshot?.renderAudit?.hub?.visibility,
                      rightSnapshot?.renderAudit?.hub?.visibility
                    )}
                    {renderDiffRow(
                      '  ↳ opacity',
                      leftSnapshot?.renderAudit?.hub?.opacity,
                      rightSnapshot?.renderAudit?.hub?.opacity
                    )}
                    {renderDiffRow(
                      '  ↳ transform',
                      leftSnapshot?.renderAudit?.hub?.transform,
                      rightSnapshot?.renderAudit?.hub?.transform
                    )}
                    {renderDiffRow(
                      '  ↳ filter',
                      leftSnapshot?.renderAudit?.hub?.filter,
                      rightSnapshot?.renderAudit?.hub?.filter
                    )}
                    {renderDiffRow(
                      '  ↳ contain',
                      leftSnapshot?.renderAudit?.hub?.contain,
                      rightSnapshot?.renderAudit?.hub?.contain
                    )}
                    {renderDiffRow(
                      '  ↳ isolation',
                      leftSnapshot?.renderAudit?.hub?.isolation,
                      rightSnapshot?.renderAudit?.hub?.isolation
                    )}
                    {renderDiffRow(
                      '  ↳ overflow',
                      leftSnapshot?.renderAudit?.hub?.overflow,
                      rightSnapshot?.renderAudit?.hub?.overflow
                    )}
                    ...
                    >
                      ...
                      >
                        PIXEL VISIBILITY PROBES
                      </td>
                    </tr>
                    {renderDiffRow(
                      'Screen Visually Empty (All Points)',
                      String(!!leftSnapshot?.visualProbe?.allEmpty),
                      String(!!rightSnapshot?.visualProbe?.allEmpty)
                    )}
                    ...
                    {['center', 'topLeft', 'topRight', 'bottomLeft', 'bottomRight'].map((pt) => {
                      ...
                      const leftVal = leftPt
                        ? `status:${leftPt.status} color:${leftPt.color} el:${leftPt.element} hasTxt:${leftPt.hasContent}`
                        : 'N/A';
                      const rightVal = rightPt
                        ? `status:${rightPt.status} color:${rightPt.color} el:${rightPt.element} hasTxt:${rightPt.hasContent}`
                        : 'N/A';

                      return renderDiffRow(
                        `Point: ${pt} (${leftPt?.point || '?'})`,
                        leftVal,
                        rightVal,
                        `probe-${pt}`
                      );
                    ...
                    {renderDiffRow(
                      'DOM State',
                      leftSnapshot?.paintVerification?.domExists !== undefined
                        ? leftSnapshot.paintVerification.domExists
                          ? 'Exists'
                          : 'Missing'
                        : 'N/A',
                      rightSnapshot?.paintVerification?.domExists !== undefined
                        ? rightSnapshot.paintVerification.domExists
                          ? 'Exists'
                          : 'Missing'
                        : 'N/A'
                    )}
                    {renderDiffRow(
                      'Paint State',
                      leftSnapshot?.paintVerification?.paintState || 'N/A',
                      rightSnapshot?.paintVerification?.paintState || 'N/A'
                    )}
                    {renderDiffRow(
                      'Black Pixels %',
                      leftSnapshot?.paintVerification?.blackPercent !== undefined
                        ? `${leftSnapshot.paintVerification.blackPercent}%`
                        : 'N/A',
                      rightSnapshot?.paintVerification?.blackPercent !== undefined
                        ? `${rightSnapshot.paintVerification.blackPercent}%`
                        : 'N/A'
                    )}
                    {renderDiffRow(
                      'Pixel Histogram',
                      leftSnapshot?.paintVerification?.histogram
                        ? `B:${leftSnapshot.paintVerification.histogram.black} D:${leftSnapshot.paintVerification.histogram.dark} M:${leftSnapshot.paintVerification.histogram.mid} Br:${leftSnapshot.paintVerification.histogram.bright}`
                        : 'N/A',
                      rightSnapshot?.paintVerification?.histogram
                        ? `B:${rightSnapshot.paintVerification.histogram.black} D:${rightSnapshot.paintVerification.histogram.dark} M:${rightSnapshot.paintVerification.histogram.mid} Br:${rightSnapshot.paintVerification.histogram.bright}`
                        : 'N/A'
                    )}
                    {renderDiffRow(
                      'Visually Black & DOM Exists',
                      leftSnapshot?.paintVerification
                        ? String(
                            leftSnapshot.paintVerification.paintState === 'visually_black' &&
                              leftSnapshot.paintVerification.domExists
                          )
                        : 'N/A',
                      rightSnapshot?.paintVerification
                        ? String(
                            rightSnapshot.paintVerification.paintState === 'visually_black' &&
                              rightSnapshot.paintVerification.domExists
                          )
                        : 'N/A'
                    )}
                    ...
                    >
                      ...
                      >
                        DOM BOUNDS AUDIT (getBoundingClientRect)
                      </td>
                    </tr>
                    {Object.keys(leftSnapshot?.bounds || rightSnapshot?.bounds || {}).map(
                      (selector) => {
                        const beforeBounds = leftSnapshot?.bounds?.[selector];
                        const afterBounds = rightSnapshot?.bounds?.[selector];
                        const beforeVal = beforeBounds?.exists
                          ? `l:${beforeBounds.left} t:${beforeBounds.top} w:${beforeBounds.width} h:${beforeBounds.height}`
                          : 'Not Found';
                        const afterVal = afterBounds?.exists
                          ? `l:${afterBounds.left} t:${afterBounds.top} w:${afterBounds.width} h:${afterBounds.height}`
                          : 'Not Found';
                    ...
                    {Object.keys(leftSnapshot?.elements || rightSnapshot?.elements || {}).map(
                      (elKey) => {
                        ...
                        keysToCompare.forEach((prop) => {
                          ...
                          rows.push(
                            renderDiffRow(`  ↳ ${prop}`, beforeVal, afterVal, `${elKey}-${prop}`)
                          );
                        });
        ...
        >
          ...
          >
            ...
            <ActionButton
              variant="copy"
              onClick={copyFilteredDOM}
              style={{ ...panicMenuItemStyle, gridColumn: 'span 2' }}
            >
              Copy Filtered DOM
            </ActionButton>
            <ActionButton variant="copy" onClick={copyEverything} style={panicMenuItemStyle}>
              All
            </ActionButton>
            <ActionButton variant="copy" onClick={copyShortSummary} style={panicMenuItemStyle}>
              Brief
            </ActionButton>
            <ActionButton variant="copy" onClick={copyDOMSnapshot} style={panicMenuItemStyle}>
              DOM
            </ActionButton>
            <ActionButton variant="copy" onClick={copyTopmostElements} style={panicMenuItemStyle}>
              Top
            </ActionButton>
            <ActionButton
              variant="copy"
              onClick={copyNavigationTimeline}
              style={panicMenuItemStyle}
            >
              Nav
            </ActionButton>
            <ActionButton variant="copy" onClick={copyLogsWarnings} style={panicMenuItemStyle}>
              Logs
            </ActionButton>
            <ActionButton variant="copy" onClick={copyLastCapture} style={panicMenuItemStyle}>
              Capture
            </ActionButton>
            <ActionButton variant="copy" onClick={copyOverlayHealth} style={panicMenuItemStyle}>
              Health
            </ActionButton>
          </div>
          ...
          <button
            onClick={() =>
              runRecoveryAction('Force Remove Overlays', forceRemoveAllFullscreenOverlays)
            }
            style={{
              ...panicMenuItemStyle,
              background: 'rgba(255, 255, 255, 0.04)',
      ...
      >
        <div id="livex-emergency-panel" className="flex flex-col h-full w-full">
          ...
          >
            <div>
              ...
              >
                ...
                {lastFailedTimeline &&
                  lastFailedTimeline.appVersion &&
                  lastFailedTimeline.appVersion !== NATIVE_VERSION && (
            ...
            {(
              ...
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  borderBottom:
                    activeTab === tab ? '2px solid rgb(168, 85, 247)' : '2px solid transparent',
          ...
          >
            ...
            {activeTab === 'blockers' && (
              <div>
                <h3 style={{ margin: '0 0 10px 0', color: 'rgb(168, 85, 247)', fontSize: '12px' }}>
                  ELEMENT HIT-TEST AT 5 KEY COORDINATES
                </h3>
...
`}
            ...
            {activeTab === 'recovery' && (
              <div>
                <h3 style={{ margin: '0 0 4px 0', color: 'rgb(168, 85, 247)', fontSize: '12px' }}>
                  FORCE RECOVERY ACTIONS
                </h3>
                <p
                  style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '10px' }}>
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Resets appMode to hub, unlinks transitions, and increments render key
                    </div>
                  </button>

                  <button
                    onClick={() =>
                      runRecoveryAction('Force Transition Unlock', forceTransitionUnlock)
                    }
                    style={{
                      padding: '12px',
                      background: 'rgba(255,255,255,0.04)',
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Unlocks transition lock boolean and fires zoom reset event
                    </div>
                  </button>

                  <button
                    onClick={() =>
                      runRecoveryAction('Force Clear Navigation Locks', forceClearNavigationLocks)
                    }
                    style={{
                      padding: '12px',
                      background: 'rgba(255,255,255,0.04)',
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Clears Capacitor and store transition variables
                    </div>
                  </button>

                  <button
                    onClick={() =>
                      runRecoveryAction('Force Clear Motion Wrappers', forceClearMotionWrappers)
                    }
                    style={{
                      padding: '12px',
                      background: 'rgba(255,255,255,0.04)',
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Locates and removes .sc-subapp-wrapper nodes directly from DOM
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Triggers increment on StudioHub key directly
                    </div>
                  </button>

                  <button
                    onClick={() =>
                      runRecoveryAction('Force Re-render Entire App', forceRerenderEntireApp)
                    }
                    style={{
                      padding: '12px',
                      background: 'rgba(239, 68, 68, 0.2)',
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Remounts root App layout (simulates fresh restart without page reload)
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Finds and deletes fixed/absolute overlays &gt;= 90% size directly from DOM
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      Sets opacity=1, visibility=visible, display=block on all Hub containers
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    style={{
                      padding: '12px',
                      background: isBlackScreenSimulated
                        ? 'rgba(239, 68, 68, 0.4)'
                        : 'rgba(255, 255, 255, 0.04)',
                      border: isBlackScreenSimulated
                  ...
                  >
                    ...
                    <div style={{ fontSize: '9px', opacity: 0.7, marginTop: '2px' }}>
                      {isBlackScreenSimulated
                        ? '🔴 Active (Tap to remove)'
                        : '🖤 Inactive (Tap to simulate)'}
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                {hubRootMissingCapture && (
                  ...
                  >
                    ...
                    >
                      <strong style={{ color: '#ff8a8a', fontSize: '10px' }}>
                        ⚠️ HUB_ROOT_MISSING CAPTURE
                      </strong>
...
activeSubApp:    ${hubRootMissingCapture.activeSubApp}`}
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button
                        onClick={() =>
                          copyToClipboard(
                            JSON.stringify(hubRootMissingCapture, null, 2),
                            'HUB_ROOT_MISSING_CAPTURE'
                          )
                        }
                        style={{
                          flex: 1,
                ...
                >
                  <h3 style={{ margin: 0, color: 'rgb(168, 85, 247)', fontSize: '12px' }}>
                    BLACK SCREEN AUTO CAPTURES ({autoCaptures.length}/20)
                  </h3>
                  <button
                    onClick={() => {
                      try {
                          ...
                          >
                            ...
                            <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                              <button
                                onClick={() =>
                                  copyToClipboard(
                                    JSON.stringify(entry.fullPayload.diagnostics, null, 2),
                                    'Diagnostics Data'
                                  )
                                }
                                style={{
                              ...
                              <button
                                onClick={() =>
                                  copyToClipboard(entry.fullPayload.domTree, 'DOM Tree')
                                }
                                style={{
                                  flex: 1,
                                  padding: '6px',
            ...
            {activeTab === 'dom' && (
              <div>
                ...
                >
                  This lists all currently visible nodes in the DOM, including z-index, opacity, and
                  dimensions.
                </p>
                <pre
                  style={{
