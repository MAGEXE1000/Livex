...
try {
  ...
  assert(
    ...
    'Web App router must not use Android SharedNavigationBar shell.'
  );
  assert(
    !webAppSrc.includes('safe-area-inset-top'),
    'Web App must not hardcode Android native safe area top insets.'
  );
  ...
  assert(
    !androidAppSrc.includes('StudioLandingPage'),
    'Android App router must exclude StudioLandingPage.'
  );
  assert(
    androidAppSrc.includes('SharedNavigationBar'),
    'Android App router must use Android SharedNavigationBar shell.'
  );
  assert(
  ...
  assert(
    capConfigSrc.includes("webDir: '../../dist/android-web'"),
    'Capacitor config must target dist/android-web output directory.'
  );
  ...
  execSync('node --import ./scripts/esm-resolver.js scripts/run-updater-regression-tests.mjs', {
    stdio: 'inherit',
    cwd: repoRoot,
  });
  ...
  execSync('node --import ./scripts/esm-resolver.js scripts/run-navigation-regression-tests.mjs', {
    stdio: 'inherit',
    cwd: repoRoot,
  });
  console.log('✓ Navigation Regression Tests passed.');

  // 7e. Run Navigation Core Regression Tests
  console.log('Running Navigation Core Regression Tests...');
  execSync('node --import ./scripts/esm-resolver.js scripts/run-navigation-core-tests.mjs', {
    stdio: 'inherit',
    cwd: repoRoot,
  });
  console.log('✓ Navigation Core Regression Tests passed.');
  ...
  for (const { input, expected } of tests) {
    ...
    assert.strictEqual(
      res,
      expected,
      `Normalization failed: "${input}" -> expected "${expected}", got "${res}"`
    );
  }
