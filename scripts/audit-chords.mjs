...
if (!fs.existsSync(chordsJsPath)) {
  console.error(
    `Error: Transpiled chords.js not found at ${chordsJsPath}. Please build packages first.`
  );
  process.exit(1);
}

// Dynamically import chords JS using pathToFileURL for Windows compatibility
const { getChordByName, normalizeChordName, getAllChords } = await import(
  pathToFileURL(chordsJsPath).href
);
...
for (const item of aliases) {
  ...
  if (normalized !== item.expected) {
    console.error(
      `[FAIL] Normalization fail: "${item.from}" normalized to "${normalized}", expected "${item.expected}"`
    );
  }
...
console.log(
  `- Diagrams Resolved:  ${resolvedCount} (${((resolvedCount / totalTested) * 100).toFixed(1)}%)`
);
