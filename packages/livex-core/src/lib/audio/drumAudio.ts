...
export function getSoundForVariation(
  ...
  if (inst === 'hihat-closed') {
    if (variation === 'open')
      return soundMap['hihat-open'] ?? kitDefaults['hihat-open'] ?? 'hh-o-short';
    if (variation === 'pedal')
      return soundMap['hihat-foot'] ?? kitDefaults['hihat-foot'] ?? 'hh-f-std';
  }
  ...
  if (inst === 'crash' && variation === 'ride')
    return soundMap['ride'] ?? kitDefaults['ride'] ?? 'ride-std';
...
export function setHumanizeVelocity(on: boolean): void {
  _humanizeVelocity = on;
}

function getVelocityGain(rawVelocity: number | undefined): number {
  const base =
    typeof rawVelocity === 'number' && Number.isFinite(rawVelocity)
      ? rawVelocity
      : VELOCITY_NEUTRAL;
  const jitter = _humanizeVelocity ? (Math.random() * 2 - 1) * HUMANIZE_VEL_JITTER : 0;
  const v = Math.max(1, Math.min(127, base + jitter));
  return v / VELOCITY_NEUTRAL;
}

// ── Kit definitions ───────────────────────────────────────────────────────────
export const KIT_DEFAULTS: Record<
  KitType,
  {
    label: string;
    description: string;
...
> = {
  ...
  jazz: {
    ...
    description:
      'Snare-03 variant · soft hi-hat · early-room reflections · intimate jazz character',
    soundMap: {
      kick: 'kick-tight',
      snare: 'snare-brush',
  ...
  rmm: {
    ...
    description:
      'Commercial-grade studio recording · public domain · 20+ velocity layers per instrument',
    soundMap: {
      kick: 'kick-acoustic',
      snare: 'snare-fat',
      'hihat-closed': 'hh-c-crisp',
  ...
  chrome: {
    ...
    description:
      'Acoustic recording by Chris Wilson (cwilso / Google) · used in original Web Audio API demos',
    soundMap: {
      kick: 'kick-acoustic',
      snare: 'snare-crack',
      'hihat-closed': 'hh-c-crisp',
  ...
  house: {
    label: 'House Kit',
    description:
      'Premium studio recording · 5 velocity layers · 7 round-robin variations · 4 mic positions',
    soundMap: {
      kick: 'kick-acoustic',
      snare: 'snare-fat',
      'hihat-closed': 'hh-c-crisp',
  ...
  rock: {
    label: 'Rock Kit',
    description: 'Big punchy kick, fat cracking snare',
    soundMap: {
      kick: 'kick-acoustic',
      snare: 'snare-fat',
      'hihat-closed': 'hh-c-crisp',
  ...
  vintage: {
    label: "Vintage '60s",
    description: 'Woodsy warm tones, open resonance',
    soundMap: {
      kick: 'kick-std',
      snare: 'snare-fat',
      'hihat-closed': 'hh-c-crisp',
  ...
  studio: {
    label: 'Studio A',
    description: 'Clean compressed bright studio kit',
    soundMap: {
      kick: 'kick-tight',
      snare: 'snare-crack',
  ...
  linn: {
    label: 'LinnDrum',
    description: '1982 sample-based drum machine',
    soundMap: {
      kick: 'kick-std',
      snare: 'snare-crack',
      'hihat-closed': 'hh-c-tight',
  ...
  funk: {
    label: 'Funk Kit',
    description: 'Tight snappy groove machine',
    soundMap: {
      kick: 'kick-tight',
      snare: 'snare-crack',
      'hihat-closed': 'hh-c-tight',
  ...
  techno: {
    label: 'Techno Kit',
    description: 'Hard punching industrial rave',
    soundMap: {
      kick: 'kick-tight',
      snare: 'snare-crack',
      'hihat-closed': 'hh-c-tight',
  ...
  stark: {
    label: 'Stark Industrial',
    description: 'Cold metallic machine sounds',
    soundMap: {
      kick: 'kick-808',
      snare: 'snare-rimshot',
      'hihat-closed': 'hh-c-tight',
...
const KIT_ACOUSTIC_CFG: Partial<Record<KitType, Partial<Record<DrumInstrument, KitInstCfg>>>> = {
  ...
  ludwig: {
    'tom-high': { rate: 0.74, gain: 1.0 }, // 1.35 × 0.74 ≈ 1.00 — native pitch
    'tom-floor': { rate: 1.25, gain: 1.0 }, // 0.80 × 1.25 = 1.00 — native pitch
  },
  // Pearl Master Studio (Brushed) — intimate small-room, soft room bloom
  jazz: {
    kick: { rate: 0.87, gain: 0.8, roomMs: 18 },
    snare: { rate: 0.9, gain: 0.7, roomMs: 14 },
    'hihat-closed': { rate: 0.96, gain: 0.56 },
    'hihat-open': { rate: 0.96, gain: 0.6 },
    'hihat-foot': { rate: 0.96, gain: 0.5 },
    'tom-high': { rate: 0.63, gain: 0.8, roomMs: 16 },
    'tom-mid': { rate: 0.9, gain: 0.8, roomMs: 14 },
    'tom-floor': { rate: 1.1, gain: 0.8, roomMs: 18 },
  },
  ...
  rock: {
    ...
    'hihat-open': { rate: 1.0, gain: 1.0 },
    'hihat-foot': { rate: 1.02, gain: 1.02 },
    'tom-high': { rate: 0.74, gain: 1.12 },
    'tom-mid': { rate: 1.05, gain: 1.1 },
    'tom-floor': { rate: 1.19, gain: 1.12 },
  },
  // Vintage '60s — boomy/warm; all toms lowered & given generous early room decay
  vintage: {
    kick: { rate: 0.76, gain: 0.9, roomMs: 24 },
    snare: { rate: 0.8, gain: 0.84, roomMs: 20 },
    'hihat-closed': { rate: 0.9, gain: 0.72 },
    'hihat-open': { rate: 0.9, gain: 0.78 },
    'tom-high': { rate: 0.59, gain: 0.9, roomMs: 20 },
    'tom-mid': { rate: 0.84, gain: 0.9, roomMs: 20 },
    'tom-floor': { rate: 1.06, gain: 0.9, roomMs: 20 },
  },
  ...
  rmm: {
    kick: { rate: 1.02, gain: 1.1 },
    snare: { rate: 1.04, gain: 1.15 },
    'hihat-closed': { rate: 1.0, gain: 1.0 },
    'hihat-open': { rate: 1.0, gain: 1.0 },
    'hihat-foot': { rate: 1.0, gain: 0.9 },
    'tom-high': { rate: 1.4, gain: 1.05 }, // pitch up for hi tom
    'tom-mid': { rate: 1.0, gain: 1.05 }, // natural pitch for mid
    'tom-floor': { rate: 0.68, gain: 1.1 }, // pitch down for floor tom
    crash: { rate: 1.0, gain: 1.0 },
    ride: { rate: 1.0, gain: 0.95 },
  },
  // Chrome Web Audio Acoustic — bright, articulate; keep natural character
  chrome: {
    kick: { rate: 1.0, gain: 1.05 },
    snare: { rate: 1.0, gain: 1.0 },
    'hihat-closed': { rate: 1.05, gain: 0.95 },
    'hihat-open': { rate: 1.0, gain: 1.0 },
    'hihat-foot': { rate: 1.0, gain: 0.85 },
    'tom-high': { rate: 1.0, gain: 1.0 },
    'tom-mid': { rate: 1.0, gain: 1.0 },
    'tom-floor': { rate: 1.0, gain: 1.05 },
  },
  // House Kit — samples are native pitch/gain; toms use separate files per size
  house: {
    kick: { rate: 1.0, gain: 1.0 },
    snare: { rate: 1.0, gain: 1.0 },
    'tom-high': { rate: 1.0, gain: 1.0 },
    'tom-mid': { rate: 1.0, gain: 1.0 },
    'tom-floor': { rate: 1.0, gain: 1.0 },
  },
};
...
function getCtx(): { ctx: AudioContext; dest: AudioNode } {
  if (!_ctx) {
    ...
    _compressor.attack.value = 0.01;
    _compressor.release.value = 0.4;
...
const KIT_SAMPLE_URLS: Record<KitType, Partial<Record<DrumInstrument, string[]>>> = {
  ...
  rmm: {
    kick: [
      `${OSDK}kick/kick10.wav`,
      `${OSDK}kick/kick8.wav`,
      `${PEARL}kick-01.wav`,
      `${MIDI}acoustic-kit/kick.wav`,
    ],
    snare: [
      `${OSDK}snare/snare-top10.wav`,
      `${OSDK}snare/snare-top8.wav`,
      `${PEARL}snare-01.wav`,
      `${MIDI}acoustic-kit/snare.wav`,
    ],
    'hihat-closed': [`${PEARL}hihat-closed.wav`, `${MIDI}acoustic-kit/hihat.wav`],
    'hihat-open': [`${PEARL}hihat-open.wav`, `${MIDI}acoustic-kit/hihat.wav`],
    'hihat-foot': [`${PEARL}hihat-closed.wav`, `${MIDI}acoustic-kit/hihat.wav`],
    'tom-high': [
      `${OSDK}toms/large-tom5.wav`,
      `${OSDK}toms/large-tom3.wav`,
      `${PEARL}tom-01.wav`,
      `${MIDI}acoustic-kit/tom1.wav`,
    ],
    'tom-mid': [
      `${OSDK}toms/large-tom5.wav`,
      `${OSDK}toms/large-tom3.wav`,
      `${PEARL}tom-02.wav`,
      `${MIDI}acoustic-kit/tom2.wav`,
    ],
    'tom-floor': [
      `${OSDK}toms/large-tom5.wav`,
      `${OSDK}toms/large-tom3.wav`,
      `${PEARL}tom-03.wav`,
      `${MIDI}acoustic-kit/tom3.wav`,
    ],
    crash: [
      `${OSDK}crash/crash10.wav`,
      `${OSDK}crash/crash8.wav`,
      `${PEARL}crash-01.wav`,
      `${TONEJS}CR78/crash.mp3`,
    ],
    ride: [
      `${OSDK}ride/ride-mid-in8.wav`,
      `${OSDK}ride/ride-mid-out5.wav`,
      `${PEARL}ride-01.wav`,
      `${TONEJS}CR78/ride.mp3`,
    ],
  },
...
class SamplePool {
  ...
  async loadForKit(kit: KitType, ctx: AudioContext) {
    ...
    await Promise.all(
      entries.map(async ([inst, urlList]) => {
  ...
  has(inst: DrumInstrument): boolean {
    return false;
  }
  get status(): SampleStatus {
    return this._status;
  }
...
export const HOUSE_VEL_CONFIGS: Record<HouseInstName, HouseVelConfig[]> = {
  ...
  snare: [
    ...
    { id: 'hard', label: 'Hard' },
    { id: 'medhard', label: 'Med Hi' },
    { id: 'medsoft', label: 'Med Lo' },
    { id: 'soft', label: 'Soft' },
  ],
  tom10: [
    { id: 'hard', label: 'Hard' },
...
export function setHouseKitMic(mic: HouseMic) {
  _houseKitMic = mic;
}
...
export function setRandomVariations(on: boolean): void {
  _randomVariations = on;
}
...
class CymbalPool {
  ...
  get ready(): boolean {
    return this._defaultLoaded;
  }
  get zkReady(): boolean {
    return this._zkLoaded;
  }
  ...
  private _get(k: string): AudioBuffer | undefined {
    return this._buffers.get(k);
  }
  ...
  getHHOpen(variation: NoteVariation): AudioBuffer | undefined {
    ...
    if (variation === 'accent')
      return this._get('hh-fullopen-hard') ?? this._get('hh-mostlyopen-hard');
    if (variation === 'open')
      return this._get('hh-mostlyopen-soft') ?? this._get('hh-halfopen-hard');
  ...
  getCrash(
    ...
    if (pack === 'zildjian-k') {
      if (variation === 'choke') {
        if (_randomVariations)
          return this._get(pick(['zk-crash16-choke', 'zk-crash18-choke', 'zk-splash-choke']));
        return this._get('zk-crash18-choke');
      }
      if (_randomVariations)
        return this._get(
          pick(['zk-crash16-hit', 'zk-crash18-hit', 'zk-splash-hit', 'zk-stack-hit'])
        );
  ...
  getRide(variation: NoteVariation, pack: CymbalPack): AudioBuffer | undefined {
    if (pack === 'zildjian-k') {
      ...
      if (_randomVariations)
        return this._get(pick(['zk-ride-bow', 'zk-ride-bell', 'zk-ride-edge']));
...
export function setHouseCrashModel(model: HouseCrashModel): void {
  _houseCrashModel = model;
}
...
function synthKick(ctx: AudioContext, t: number, vol: number, dest: AudioNode, id: string) {
  const P: Record<
    string,
    { sf: number; ef: number; decay: number; clickVol: number; sub: boolean }
  > = {
    'kick-std': { sf: 185, ef: 42, decay: 0.42, clickVol: 0.4, sub: false },
    'kick-tight': { sf: 230, ef: 62, decay: 0.2, clickVol: 0.55, sub: false },
    'kick-808': { sf: 82, ef: 28, decay: 1.4, clickVol: 0.1, sub: true },
...
function synthHihat(ctx: AudioContext, t: number, vol: number, dest: AudioNode, id: string) {
  const P: Record<string, { decay: number; hp: number; lp: number; nVol: number; baseF: number }> =
    {
      'hh-c-tight': { decay: 0.03, hp: 8000, lp: 16000, nVol: 0.18, baseF: 42 },
      'hh-c-crisp': { decay: 0.05, hp: 7000, lp: 15000, nVol: 0.25, baseF: 40 },
      'hh-c-loose': { decay: 0.075, hp: 6000, lp: 13000, nVol: 0.28, baseF: 38 },
      'hh-o-short': { decay: 0.18, hp: 5500, lp: 12000, nVol: 0.38, baseF: 38 },
      'hh-o-long': { decay: 0.35, hp: 5000, lp: 12000, nVol: 0.4, baseF: 38 },
      'hh-o-wash': { decay: 0.5, hp: 4400, lp: 11000, nVol: 0.5, baseF: 36 },
      'hh-f-std': { decay: 0.085, hp: 6500, lp: 11500, nVol: 0.28, baseF: 36 },
      'hh-f-splash': { decay: 0.16, hp: 5800, lp: 11000, nVol: 0.35, baseF: 36 },
    };
...
function synthCymbal(ctx: AudioContext, t: number, vol: number, dest: AudioNode, id: string) {
  const P: Record<
    string,
    { decay: number; hp: number; attackT: number; baseF: number; nVol: number }
  > = {
    'crash-std': { decay: 1.3, hp: 4200, attackT: 0.005, baseF: 36, nVol: 0.3 },
    ...
    'ride-std': { decay: 1.8, hp: 5400, attackT: 0.008, baseF: 54, nVol: 0.22 },
    'ride-bell': { decay: 0.85, hp: 7200, attackT: 0.002, baseF: 72, nVol: 0.12 },
    'ride-wash': { decay: 2.2, hp: 3900, attackT: 0.015, baseF: 30, nVol: 0.4 },
  };
...
function chokeHihat(ctx: AudioContext, newStartTime: number, chokeMs = 0.04) {
...
function playBufferChoked(
  ...
  const chokeTime = isClosed ? 0.008 : 0.04;
...
function playBufferRoomy(
  ...
  g.gain.setValueAtTime(vol * 0.2, t + refDelay);
...
function _teardownChain(entry: ChainEntry): void {
  ...
  try {
    entry.dryOut.disconnect(entry.dest);
  } catch {
    /* already disconnected */
  }
  try {
    entry.wetOut.disconnect(entry.dest);
  } catch {
    /* already disconnected */
  }
  try {
...
function buildFXChain(
  ...
  low.gain.value = clamp(fx.eqLow);

  const lowMid = ctx.createBiquadFilter();
  lowMid.type = 'peaking';
  lowMid.frequency.value = 350;
  lowMid.Q.value = 1.0;
  lowMid.gain.value = clamp(fx.eqLowMid ?? 0);

  const mid = ctx.createBiquadFilter();
  mid.type = 'peaking';
  mid.frequency.value = 2000;
  mid.Q.value = 1.2;
  mid.gain.value = clamp(fx.eqMid);

  const hi = ctx.createBiquadFilter();
  hi.type = 'highshelf';
  hi.frequency.value = 10000;
  hi.gain.value = clamp(fx.eqHigh);
  ...
  pluginChainOut.connect(dryGain);
  dryGain.connect(dest);
  pluginChainOut.connect(conv);
  conv.connect(wetGain);
  wetGain.connect(dest);

  const disposeAll = () =>
    pluginDisposers.forEach((d) => {
      try {
        d();
      } catch {}
    });
...
function playSoundAt(
  ...
  if (gateVal > 0.05 && !isHihat) {
    const g = gateVal;
    const hold = 0.05 + (1 - g) * (1 - g) * 1.7; // 1.75 s → 50 ms  (quadratic)
    const release = 0.15 + (1 - g) * 0.25; // 400 ms → 150 ms (linear)
    ...
    gateGain.gain.exponentialRampToValueAtTime(
      0.0001, // smooth close
  ...
  if (kit === 'house' && inst) {
    ...
    if (houseInst) {
      ...
      if (inst === 'hihat-closed') {
        ...
        if (buf) {
          playBufferChoked(ctx, buf, t, vol, chainInput, 'hihat-closed', 1.0);
          return;
        }
      } else if (inst === 'hihat-open') {
        const buf = cymbalPool.getHHOpen(variation);
        if (buf) {
          playBufferChoked(ctx, buf, t, vol, noteDest, 'hihat-open', 1.0);
          return;
        }
      } else if (inst === 'hihat-foot') {
        const buf = cymbalPool.getHHFoot();
        if (buf) {
          playBufferChoked(ctx, buf, t, vol, chainInput, 'hihat-foot', 1.0);
          return;
        }
      } else if (inst === 'crash') {
  ...
  else if (soundId.startsWith('hh-')) {
    const isHHClosed = soundId.startsWith('hh-c') || soundId.startsWith('hh-f');
    chokeHihat(ctx, t, isHHClosed ? 0.008 : 0.04);
    synthHihat(ctx, t, vol, chainInput, soundId);
  } else if (soundId.startsWith('tom-')) synthTom(ctx, t, vol, noteDest, soundId);
  else if (soundId.startsWith('crash-') || soundId.startsWith('ride-'))
    synthCymbal(ctx, t, vol, noteDest, soundId);
}
...
class DrumScheduler {
  ...
  private playMetronomeClickAt(time: number, isBeat1: boolean, soundType: string) {
    ...
    if (soundType === 'wood') {
      ...
      gainNode.gain.setValueAtTime(0, t);
      gainNode.gain.linearRampToValueAtTime(isBeat1 ? 0.3 : 0.2, t + 0.001);
      ...
      osc.frequency.setValueAtTime(pitch, t);
      gainNode.gain.setValueAtTime(0, t);
      gainNode.gain.linearRampToValueAtTime(isBeat1 ? 0.28 : 0.2, t + 0.003);
  ...
  private scheduleNote(step: number, time: number, drumPrefs: ReturnType<typeof useDrumStore.getState>['drumPrefs']) {
    ...
    for (const inst of DRUM_INSTRUMENTS) {
      ...
      if (variation === 'flam') {
        const graceT = Math.max(time - 0.02, (_ctx?.currentTime ?? 0) + 0.002);
