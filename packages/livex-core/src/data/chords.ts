...
const chordDatabase: Chord[] = [
  ...
  },
  {
    ...
    guitar: {
      frets: [-1, -1, 0, 2, 2, 2],
      fingers: [0, 0, 0, 1, 1, 1],
      barres: [{ fret: 2, fromString: 4, toString: 1 }],
      baseFret: 1,
    },
    piano: { keys: [2, 6, 9, 1] },
  ...
  // Db / C#
  {
    ...
    guitar: {
      frets: [-1, 4, 3, 1, 2, 1],
      fingers: [0, 4, 3, 1, 2, 1],
      barres: [{ fret: 1, fromString: 4, toString: 1 }],
      baseFret: 1,
    },
    piano: { keys: [1, 5, 8] },
  ...
  },
  {
    ...
    guitar: {
      frets: [-1, 4, 2, 2, 2, 0],
      fingers: [0, 4, 1, 2, 3, 0],
      barres: [{ fret: 2, fromString: 4, toString: 2 }],
      baseFret: 1,
    },
    piano: { keys: [1, 9, 4] },
  },
...
function normalizeSingleChord(chord: string): string {
  ...
  let clean = chord.replace(/♭/g, 'b').replace(/♯/g, '#');
  ...
  normSuffix = normSuffix.replace(/[º°]/g, 'dim').replace(/\*/g, 'aug').replace(/\+/g, 'aug');

  // Case-sensitive replacements to preserve Major (M) vs minor (m) distinction
  normSuffix = normSuffix.replace(/7M/g, 'maj7');
  if (
    normSuffix.startsWith('M') &&
    !normSuffix.startsWith('Min') &&
    !normSuffix.startsWith('Minor')
  ) {
