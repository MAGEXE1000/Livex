            ...
            <div className="space-y-0.5">
              {[
                ...
                return (
                  ...
                  >
                    <span
                      className="w-1 h-1 rounded-full flex-shrink-0 bg-current"
                      style={{ opacity: isActive ? 1 : 0.4 }}
                    />
                    <span className="truncate text-[7px] uppercase tracking-wider">
                      {item.label}
                    </span>
                  </div>
                );
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-[#050505]">
          ...
          <div className="h-9 border-b border-zinc-900 px-3 flex items-center justify-between bg-[#080808]/40 flex-shrink-0">
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-[8.5px] uppercase text-white tracking-wide">
                {appTitle}
              </span>
            </div>
    ...
    >
      <div className="w-full h-full flex text-[8px] select-none p-2.5 gap-2.5">
        ...
        <div className="w-1/3 border-r border-zinc-900/60 pr-2.5 flex flex-col gap-1.5">
          <span className="text-[7.5px] font-extrabold tracking-wider text-zinc-500 uppercase">
            Categories
          </span>
        ...
        <div className="flex-1 flex flex-col justify-between overflow-hidden">
          ...
          <div className="text-[6.5px] text-zinc-500 font-mono tracking-wider truncate">
            Fingering: T - 0 - 0 - 2 - 3 - 2
          </div>
        </div>
      </div>
    </RealAppLayoutWrapper>
    ...
    >
      <div className="w-full h-full flex text-[8px] select-none p-2.5 gap-2.5">
          ...
          <div className="flex flex-col gap-0.5 flex-1 overflow-hidden">
            {songs.map((song, i) => (
              <div
                ...
                className={`px-1.5 py-1 rounded text-[7px] font-semibold truncate ${
                  i === 0
                    ? 'bg-zinc-900 border border-zinc-800/80 text-zinc-200 font-bold'
                    : 'text-zinc-500'
                }`}
              >
                {song}
        ...
        <div className="flex-1 flex flex-col justify-between overflow-hidden">
          ...
          <div className="text-[6px] text-zinc-600 truncate border-t border-zinc-900/60 pt-1 mt-1">
            Pink Floyd · organizat.
          </div>
        </div>
      </div>
    </RealAppLayoutWrapper>
  ...
  return (
    ...
    >
      <div className="w-full h-full p-2 flex flex-col justify-between overflow-hidden">
        <span className="text-[7.5px] font-extrabold tracking-wider text-zinc-500 uppercase px-1">
          Chords Catalog
        </span>
        <div className="grid grid-cols-3 gap-1.5 flex-1 overflow-hidden mt-1 pb-1">
          {gridChords.map((c) => (
            <div
              key={c}
              className="p-1 rounded bg-zinc-900/40 border border-zinc-900 flex flex-col items-center justify-between"
            >
    ...
    >
      <div className="w-full h-full p-2 flex flex-col justify-between select-none overflow-hidden">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[6.5px] text-zinc-400 font-bold bg-zinc-900 border border-zinc-800 px-1 rounded">
            120 BPM
          </span>
        </div>
        ...
        <div className="flex-1 flex flex-col border border-zinc-900/60 rounded overflow-hidden bg-zinc-950/20">
          {insts.map((inst, rowIdx) => (
            <div
              key={inst}
              className="flex border-b border-zinc-900/40 last:border-none flex-1 items-center"
            >
              <div className="w-10 pl-1.5 text-[6.5px] font-bold text-zinc-500 border-r border-zinc-900/60 truncate uppercase tracking-wider">
                {inst}
              </div>
                    ...
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${
                          isActive
                            ? 'bg-zinc-100 shadow-sm shadow-white/10'
    ...
    >
      <div className="w-full h-full p-2.5 flex flex-col justify-between select-none overflow-hidden">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-[6.5px] text-zinc-400 font-bold bg-zinc-900 border border-zinc-800 px-1.5 py-0.5 rounded">
            FOH Rider
          </span>
        </div>
        ...
        <div className="flex-1 border border-zinc-900 rounded-lg relative bg-zinc-950/20 overflow-hidden flex items-center justify-center">
          ...
          {/* Nodes */}
          {/* Backline */}
          <div className="absolute top-2 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded border border-zinc-800 bg-zinc-900 text-zinc-400 font-bold text-[6px] uppercase tracking-wider">
            Drums
          </div>
          {/* Frontline */}
    ...
    >
      <div className="w-full h-full p-2 flex flex-col justify-between overflow-hidden">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[6px] text-zinc-400 font-mono truncate max-w-[90px]">
            Song_Stems_Main.wav
          </span>
        </div>
        ...
        <div className="flex-1 flex gap-1 justify-around items-end pb-1 overflow-hidden">
          {channels.map((ch) => (
            <div
              key={ch.name}
              className="flex flex-col items-center gap-1.5 h-full justify-end w-1/4"
            >
              ...
              <span className="text-[6px] font-semibold text-zinc-500 tracking-wider truncate w-full text-center uppercase">
                {ch.name}
              </span>
            </div>
          ))}
      ...
      <div className="w-full h-full p-2.5 flex flex-col justify-between overflow-hidden">
        ...
        <div className="flex-1 border border-zinc-900 rounded-lg relative bg-zinc-950/20 flex flex-col justify-center overflow-hidden">
          <svg viewBox="0 0 160 80" className="w-full h-full opacity-60">
            ...
            {/* Pitch wave */}
            <path
              d="M 5 45 Q 35 15, 75 55 T 155 35"
              fill="none"
              stroke="#ffffff"
              strokeWidth="1.5"
              strokeLinecap="round"
          ...
          <div className="absolute top-2 right-2 text-[7px] text-zinc-300 font-bold font-mono">
            Deviation: -2 cents
          </div>
        </div>
      </div>
...
export function PreferencesMockup() {
  ...
  return (
    ...
    >
      <div className="w-full h-full p-2.5 flex flex-col justify-between overflow-hidden">
        <span className="text-[7.5px] font-extrabold tracking-wider text-zinc-500 uppercase border-b border-zinc-900 pb-1.5 mb-1">
          Appearance Settings
        </span>
        ...
        <div className="flex-1 flex flex-col justify-between py-1">
          {rows.map((row) => (
            <div
              key={row.label}
              className="flex justify-between items-center py-1 border-b border-zinc-900/40 last:border-none"
            >
              <div>
              ...
              <span className="text-[6.5px] font-bold text-zinc-400 bg-zinc-900 border border-zinc-800 px-1.5 py-0.5 rounded">
                {row.val}
              </span>
            </div>
          ))}
