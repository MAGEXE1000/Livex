...
export interface AppInstallerPlugin {
  ...
  verifySha256(options: {
    filePath: string;
    expectedHash: string;
  }): Promise<{ matches: boolean; computedHash: string }>;
  verifyApkSha256(options: {
    filePath: string;
    expectedHash: string;
  }): Promise<{ matches: boolean; computedHash: string }>;
  ...
  getApkDetails(options: { filePath: string }): Promise<{
    packageName: string;
    versionName: string;
    versionCode: number;
    signatures: string;
  }>;
  getInstalledAppDetails(): Promise<{
    packageName: string;
    versionName: string;
    versionCode: number;
    signatures: string;
  ...
  get(target, prop, receiver) {
    if (prop === 'installApk' || prop === 'installApkDirect' || prop === 'downloadAndInstallApk') {
      return async (options: any) => {
        ...
        if (filePath.includes('mock') || filePath.includes('simulated')) {
          throw new Error(
            `[Proxy Guard] Blocked native ${String(prop)} call in simulation. Path/URL: ${filePath}`
          );
        }
  ...
  try {
    ...
    if (cleanVer) {
      ...
      if (specificRelease && specificRelease.assets) {
        const apkAsset = specificRelease.assets.find((a) => a.name.toLowerCase().endsWith('.apk'));
        if (apkAsset) {
...
export async function downloadApk(
  ...
  try {
    await AppInstaller.appendLog({
      stage: '[INSTRUMENTATION] downloadApk ENTER',
      message: `url=${url}`,
    });
...
export async function checkApkEligibility(
  filePath: string,
  allowDowngrade?: boolean
): Promise<InstallEligibility> {
  ...
  try {
    ...
    if (downloaded.debuggable && !installed.debuggable) {
      return {
        ...
        reason: 'debuggable',
        errorDetails:
          'The update package is a debuggable build and cannot be installed on top of the release version.',
        installed,
        downloaded,
      };
    }
