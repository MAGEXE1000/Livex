    ...
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: 'block', flexShrink: 0 }}>
      {WHITE.map((chroma, i) => (
        <rect
          ...
          height={wkH - 2}
          rx={2}
          fill={keys.includes(chroma) ? accentFrom : 'rgba(240,240,240,0.9)'}
      ...
      {BLACK_POS.map((pos, i) => (
        <rect
          ...
          height={bkH}
          rx={1.5}
          fill={keys.includes(BLACK_CHROMAS[i]) ? accentFrom : '#1a1a1a'}
        />
      ))}
    </svg>
  );
...
export function CustomMiniDiagram({
  chord,
  accentFrom,
}: {
  chord: CustomChord;
...
function FretboardBuilder({
  ...
  return (
    <div style={{ userSelect: 'none', touchAction: 'none' }}>
      ...
      >
        <button
          onClick={() => setBaseFret((bf) => Math.max(1, bf - 1))}
          disabled={baseFret === 1}
          ...
          style={{
            ...
            color: baseFret === 1 ? 'var(--c-text-muted)' : 'var(--c-text-primary)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            opacity: baseFret === 1 ? 0.35 : 1,
            flexShrink: 0,
          }}
        >
        ...
        >
          {baseFret === 1
            ? t.customBuilder.openFrets
            : t.customBuilder.fretsRange(baseFret, baseFret + VISIBLE - 1)}
        </span>
        {/* Barre toggle */}
        <button
          onClick={toggleBarreMode}
          className="btn-smooth"
          style={{
            padding: '5px 11px',
            borderRadius: '9999px',
            flexShrink: 0,
            background: barreMode
        ...
        <button
          onClick={() => setBaseFret((bf) => Math.min(8, bf + 1))}
          disabled={baseFret >= 8}
          className="btn-smooth"
          style={{
            width: '32px',
            ...
            borderRadius: '50%',
            background: 'var(--app-surface-high)',
            color: baseFret >= 8 ? 'var(--c-text-muted)' : 'var(--c-text-primary)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            opacity: baseFret >= 8 ? 0.35 : 1,
            flexShrink: 0,
          }}
        >
          <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>
            ...
            >
              ...
              {Array.from({ length: VISIBLE }, (_, fIdx) => {
                ...
                const barre = barres.find(
                  (b) => b.fret === fret && sIdx >= b.toString - 1 && sIdx <= b.fromString - 1
                );
                ...
                return (
                  <button
                    key={fIdx}
                    onClick={() => fret <= 12 && handleFretClick(sIdx, fret)}
                    className="btn-smooth"
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        right: 0,
                        height: '1.5px',
                        background: `rgba(160,160,160,${sIdx === 0 || sIdx === numStrings - 1 ? '0.4' : '0.25'})`,
                        top: '50%',
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: '10px' }}>
        <button
          onClick={() => onChange(new Array(numStrings).fill(0))}
          className="btn-smooth"
          style={{
            padding: '5px 14px',
            borderRadius: '9999px',
...
function PianoKeyboard({
  ...
  return (
    <div>
      ...
      {/* Keyboard */}
      <div
        style={{ position: 'relative', userSelect: 'none', touchAction: 'none', height: '88px' }}
      >
        {/* White keys */}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isOn
                    ? `linear-gradient(180deg, ${accent.from}99, ${accent.from})`
      ...
      >
        ...
        {pianoKeys.length === 0 && (
          <span style={{ fontFamily: 'Inter', fontSize: '11px', color: 'var(--c-text-muted)' }}>
...
export default function CustomChordBuilder({
  ...
  const chromaticIndices = useMemo(() => {
    ...
    const openNotes = OPEN_NOTES[instrument];
    return frets.flatMap((arr, i) => arr.filter((f) => f >= 0).map((f) => (openNotes[i] + f) % 12));
  }, [instrument, frets, pianoKeys]);
  ...
  const previewData: GuitarChordData | null =
    instrument === 'guitar'
      ? (() => {
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              {suggested && (
                ...
                >
                  <span
                    className="material-symbols-outlined"
                    style={{ fontSize: '15px', color: resolvedAccent.from, flexShrink: 0 }}
                  >
                    auto_fix_high
                  </span>
                  <span
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  placeholder={
                    suggested
                      ? t.customBuilder.detected(suggested)
                      : t.customBuilder.namePlaceholder
                  }
                  style={{
                    width: '100%',
                    background: 'var(--app-surface)',
                  ...
                  onFocus={(e) => {
                    e.target.style.borderColor = resolvedAccent.from;
                  }}
                  onBlur={(e) => {
                    e.target.style.borderColor = 'rgba(72,72,72,0.15)';
                  }}
                />
              </div>
            </div>
