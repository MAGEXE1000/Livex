...
function _scFallback(query) {
  ...
  if (/thank/.test(q))
    return 'Happy to help. Got another question about your stage setup or signal chain?';
...
const _SC_INST_MAP = [
  ...
  {
    kw: ['acoustic drum', 'drum kit', 'drummer', 'drums', 'drum'],
    type: 'Acoustic Drums',
    name: 'Drum Kit',
    icon: 'drum',
  },
  {
    kw: ['electronic drum', 'e-drum', 'edrum', 'electric drum'],
    type: 'Electronic Drums',
    name: 'E-Drums',
    icon: 'cx-edrum',
  },
  {
    kw: ['lead guitar', 'rhythm guitar', 'electric guitar', 'elec guitar', 'guitarist'],
    type: 'Electric Guitar',
    name: 'Elec Guitar',
    icon: 'cx-elec-guitar',
  },
  {
    kw: ['acoustic guitar', 'acoustic guit'],
    type: 'Acoustic Guitar',
    name: 'Acou Guitar',
    icon: 'guitar',
  },
  {
    kw: ['bass guitar', 'bass player', 'bassist', 'bass'],
    type: 'Bass Guitar',
    name: 'Bass Guitar',
    icon: 'cx-bass-guitar',
  },
  {
    kw: ['keyboard', 'keyboardist', 'keys', 'pianist', 'piano'],
    type: 'Keyboard DI',
    name: 'Keyboard',
    icon: 'piano',
  },
  ...
  {
    kw: ['violin', 'violinist', 'viola', 'cello', 'strings'],
    type: 'String Instrument',
    name: 'Strings',
    icon: 'cx-violin',
  },
  {
    kw: ['trumpet', 'trombone', 'flugelhorn', 'brass', 'horn player'],
  ...
  {
    kw: ['percussion', 'percussionist', 'bongo', 'conga', 'timbale'],
    type: 'Percussion',
    name: 'Percussion',
    icon: 'cx-percussion',
  },
  ...
  {
    kw: ['laptop', 'playback', 'backing track', 'click track'],
...
function _scIsPlotIntent(text) {
  ...
  return (
    /(create|make|generate|build|set up|setup|draw|add|put).{0,20}(plot|stage|my band|elements?)/i.test(
      q
    ) ||
    /(plot|stage).{0,20}(for me|for my|my band|us)/i.test(q) ||
    /(i have|i've got|we have|we've got|i need a plot for|generate for).{0,40}(vocalist|drummer|guitarist|bassist|keyboardist|drummer|violin|trumpet|saxophone|sax|brass|synth|drum|guitar|bass|keys|keyboard|cajon|percussion)/i.test(
      q
    ) ||
...
function _scParseInstruments(text) {
  ...
  for (const entry of _SC_INST_MAP) {
    for (const kw of entry.kw) {
      ...
      const re = new RegExp(
        '(\\d+|' +
          Object.keys(_SC_NUM_WORDS).join('|') +
          ')\\s+(?:\\w+\\s+){0,2}' +
          kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
      );
      const m = q.match(re);
  ...
  try {
    ...
    instruments.forEach((inst, idx) => {
      ...
      const el = {
        ...
        name: libItem.name,
        label: (
          (libItem.nameKey && typeof T === 'function' ? T(libItem.nameKey) : null) || libItem.name
        ).toUpperCase(),
        icon: libItem.icon,
        type: libItem.type,
        x: W / 2,
        y: H / 2, // overwritten by _smartPlaceElements
        rotation: 0,
        scale: 100,
        channelId: 'CH-' + chNum,
        source: '',
    ...
    console.error('[StageMind plot]', e);
    return false;
  }
}
...
function smSendMessage() {
  ...
  if (typeof offlineModeEnabled !== 'undefined' && offlineModeEnabled) {
    ...
    smConversation.push({
      role: 'assistant',
      content:
        '⚡ **Offline Mode is on.** The AI Assistant is disabled. Turn off Offline Mode in Settings to use it.',
    });
    _smRender();
    _smScrollBottom();
    return;
    ...
    if (instruments.length > 0) {
      ...
      if (ok) {
        ...
        const reply =
          ...
          ' to your stage (' +
          summary +
          ') and arranged them into their standard positions. You can drag any element to fine-tune the layout, rename them in the properties panel, or use Auto-Arrange from the toolbar to re-sort at any time.';
        _scSimulateStream(
          reply,
          (chunk) => {
    ...
    var btns = qs
      .map(function (q, qi) {
        ...
        return (
          ...
          '<button class="sm-suggestion" style="flex:1;" data-q="' +
          q.replace(/"/g, '&quot;') +
          '" onclick="smQuickAsk(this.dataset.q)">' +
          q +
          '</button>' +
          '<button onclick="var el=document.getElementById(\'' +
          sid +
          '\');if(el)el.remove();" style="background:none;border:none;cursor:pointer;color:#484847;font-size:15px;line-height:1;padding:2px 5px;flex-shrink:0;transition:color .12s;" onmouseover="this.style.color=\'#9a9a9a\'" onmouseout="this.style.color=\'#484847\'" title="Dismiss">×</button>' +
          '</div>'
        );
      })
      .join('');
  ...
  smConversation.forEach((msg, i) => {
    ...
    if (msg.role === 'user') {
      div.innerHTML = DOMPurify.sanitize(
        `<div class="sm-bubble sm-bubble-user">${_smEscape(msg.content)}</div>`
      );
    } else {
...
function _buildStageContext() {
  ...
  const els = (state.elements || [])
    .map((e) => `${e.name}${e.type ? ' (' + e.type + ')' : ''}${e.chid ? ' ch:' + e.chid : ''}`)
    .join(', ');
...
const GAIN_PROFILES = {
  'Dynamic Mic': {
    ...
    max: -12,
    label: 'Dynamic Mic',
    note: 'Preamp gain 50-60dB typical. Works best on loud sources.',
  },
  'Condenser Mic': {
    rec: -24,
    min: -32,
    max: -16,
    label: 'Condenser',
    note: 'Requires +48V phantom power. Handle with care near high SPL.',
  },
  'Instrument Mic': {
    rec: -22,
    min: -30,
    max: -14,
    label: 'Inst Mic',
    note: 'Close-mic position. Pad if needed for loud amps.',
  },
  'Instrument Clip': {
    rec: -20,
    min: -28,
    max: -12,
    label: 'Clip Mic',
    note: 'Position away from rim strikes to reduce bleed.',
  },
  'PZM Mic': {
    rec: -26,
    min: -34,
    max: -18,
    label: 'PZM Boundary',
    note: 'High sensitivity. Place on flat surface for best results.',
  },
  'Wireless Mic': {
    rec: -20,
    min: -28,
    max: -12,
    label: 'Wireless',
    note: 'Set TX output to match system. Coordinate frequencies pre-show.',
  },
  'Electric Guitar': {
    rec: -16,
    min: -24,
    max: -8,
    label: 'Electric Gtr',
    note: 'Line or instrument level depending on DI/amp routing.',
  },
  'Acoustic Guitar': {
    rec: -24,
    min: -32,
    max: -16,
    label: 'Acoustic Gtr',
    note: 'Low output — use high-impedance DI or instrument mic.',
  },
  'Bass Guitar': {
    rec: -14,
    min: -22,
    max: -6,
    label: 'Bass Guitar',
    note: 'High output. Watch for low-end clipping on preamp.',
  },
  'Guitar Amplifier': {
    rec: -16,
    min: -22,
    max: -8,
    label: 'Amp Mic',
    note: 'Dynamic mic close-miked. May need pad at high stage volumes.',
  },
  'Bass Amplifier': {
    rec: -14,
    min: -20,
    max: -6,
    label: 'Bass Amp Mic',
    note: 'Use a large-diaphragm dynamic or DI split.',
  },
  'Keyboard DI': {
    rec: -10,
    min: -18,
    max: -2,
    label: 'Keyboard',
    note: 'Line level output. Minimal preamp gain required.',
  },
  Synthesizer: {
    rec: -10,
    min: -18,
    max: -2,
    label: 'Synthesizer',
    note: 'Line level. Hot patches may clip — check peak output.',
  },
  'Acoustic Drums': {
    rec: -20,
    min: -28,
    max: -12,
    label: 'Drum Kit',
    note: 'Kick mic typically hotter. Set channels individually.',
  },
  'Electronic Drums': {
    rec: -12,
    min: -18,
    max: -4,
    label: 'E-Drums',
    note: 'Line level from module. Stereo DI recommended.',
  },
  Percussion: {
    rec: -22,
    min: -30,
    max: -14,
    label: 'Percussion',
    note: 'Overhead or clip mic. Loud transients — watch peaks.',
  },
  Cajón: {
    rec: -20,
    min: -28,
    max: -12,
    label: 'Cajón',
    note: 'Single close mic or internal pickup. Front hole for thump.',
  },
  'Loop Station': {
    rec: -10,
    min: -16,
    max: -2,
    label: 'Looper',
    note: 'Line out from device. Stereo DI if stereo loop output.',
  },
  'Playback Device': {
    rec: -10,
    min: -16,
    max: -2,
    label: 'Playback',
    note: 'Stereo line level. Set device output to -10dBu nominal.',
  },
  'In-Ear Monitor': {
    rec: -18,
    min: -24,
    max: -6,
    label: 'IEM Send',
    note: 'Aux send to IEM TX. Start at unity (-18dBFS) and adjust.',
  },
  'Floor Wedge': {
    rec: -18,
    min: -24,
    max: -6,
    label: 'Wedge Send',
    note: 'Aux send level pre-fader. Set flat, then ring out.',
  },
  'DI Box': {
    rec: -14,
    min: -22,
    max: -6,
    label: 'DI Box',
    note: 'Passive DI: 20-30dB instrument level. Active: 0-10dB gain.',
  },
};
...
function updateGainHelper(el) {
  ...
  const pct = Math.max(
    0,
    Math.min(100, ((level - profile.min) / (profile.max - profile.min)) * 100)
  );
...
function adjustGain(delta) {
  ...
  var el = state.elements.find(function (e) {
...
const PA_SPEAKER_CONFIGS = {
  'Main PA Left': {
    angle: 90,
    spread: 75,
    color: 'rgba(255,116,57,0.15)',
    stroke: 'rgba(255,116,57,0.5)',
  },
  'Main PA Right': {
    angle: 90,
    spread: 75,
    color: 'rgba(197,255,201,0.15)',
    stroke: 'rgba(197,255,201,0.5)',
  },
  ...
  'Side Fill': {
    angle: 90,
    spread: 50,
    color: 'rgba(200,162,255,0.12)',
    stroke: 'rgba(200,162,255,0.4)',
  },
  'Front Fill Speaker': {
    angle: 90,
    spread: 30,
    color: 'rgba(122,175,255,0.1)',
    stroke: 'rgba(122,175,255,0.35)',
  },
  'Delay Speaker Tower': {
    angle: 90,
    spread: 50,
  ...
  'Stage Sub-Woofer': {
    angle: 90,
    spread: 110,
    color: 'rgba(122,175,255,0.06)',
    stroke: 'rgba(122,175,255,0.2)',
  },
  'Guitar Amplifier': {
    angle: 90,
    spread: 50,
...
function _injectSoundCoverageUI() {
  ...
  const el =
    typeof state !== 'undefined' ? state.elements.find((e) => e.id === state.selectedId) : null;
...
function updateConflictBanner() {
  ...
  const lines = conflicts
    .map((c) => `${c.ch}: "${c.a.label || c.a.name}" & "${c.b.label || c.b.name}"`)
    .join('\n');
...
function applyLayers() {
  ...
  state.elements.forEach((el) => {
    ...
    for (const [key, layer] of Object.entries(LAYERS)) {
      ...
      if (layer.types.includes(el.type || '') && !layer.visible) {
        show = false;
        break;
      }
    }
...
function msDeleteSelected() {
  ...
  state.elements = state.elements.filter(function (e) {
    return !multiSel.has(e.id);
  });
  state.connections = state.connections.filter(function (c) {
    return !multiSel.has(c.from) && !multiSel.has(c.to);
  });
  multiSel.clear();
...
function msGroupSelected() {
  ...
  if (typeof showToast === 'function')
    showToast('Group active — drag any selected element to move them all');
}
...
document.addEventListener(
  ...
  function (e) {
    ...
    var target = e.target;
    var onBg =
      target === document.getElementById('stage-canvas') ||
...
function runAutoWire() {
  ...
  WIRE_RULES.forEach((rule) => {
    ...
    froms.forEach((f) => {
      ...
      const exists = state.connections.some(
        (c) => (c.from === f.id && c.to === nearest.id) || (c.from === nearest.id && c.to === f.id)
      );
      if (!exists) {
        state.connections.push({ from: f.id, to: nearest.id });
        added++;
      }
    });
  });
  ...
  if (added > 0 && typeof showToast === 'function')
    showToast(`Auto Wiring: ${added} connection${added > 1 ? 's' : ''} added`);
}
...
function _smGetBehavior() {
  try {
    return JSON.parse(localStorage.getItem(SM_BEHAVIOR_KEY) || '{}');
  } catch {
    return {};
  }
}
function _smSaveBehavior(b) {
  try {
    localStorage.setItem(SM_BEHAVIOR_KEY, JSON.stringify(b));
  } catch {}
}
...
function smAnalyzeLayout() {
  ...
  const canvasW =
    (state.canvasW > 0 ? state.canvasW : 0) || (window.innerWidth > 0 ? window.innerWidth : 800);
  const canvasH =
    (state.canvasH > 0 ? state.canvasH : 0) || (window.innerHeight > 0 ? window.innerHeight : 600);
  ...
  for (let i = 0; i < els.length; i++) {
    for (let j = i + 1; j < els.length; j++) {
      if (
        Math.abs(els[i].x - els[j].x) < OVERLAP_ABS &&
  ...
  const hasOutput = els.some((e) =>
    /foh|pa speaker|main speaker|monitor|wedge|iem/i.test(e.name || '')
  );
  ...
  if (canvasW > 0 && canvasH > 0) {
    const offStage = els.filter(
      (e) => e.x < -30 || e.x > canvasW + 30 || e.y < -30 || e.y > canvasH + 30
    );
    if (offStage.length) {
      issues.push({
        severity: 'warning',
        ...
        desc:
          offStage
            ...
            .join(', ') + " — off-screen elements won't appear on PDF exports.",
        action: null,
      });
    }
  }
...
function smAnalyzeConflicts() {
  ...
  if (dupes.length) {
    issues.push({
      ...
      desc:
        dupes
          .slice(0, 3)
          .map(([ch, names]) => `${ch}: ${names.join(', ')}`)
          .join(' · ') + (dupes.length > 3 ? ` +${dupes.length - 3} more` : ''),
    ...
    issues.push({
      ...
      desc:
        noChannel
          .slice(0, 3)
          .map((e) => e.label || e.name)
          .join(', ') + (noChannel.length > 3 ? ` +${noChannel.length - 3} more` : ''),
      action: null,
    });
  }
    ...
    issues.push({
      ...
      desc:
        unconnected
          .slice(0, 3)
          .map((e) => e.label || e.name)
          .join(', ') + ' — connect to FOH or a snake for complete signal routing.',
      action: null,
    });
  }
  ...
  if (phantomPassiveRibbon.length) {
    issues.push({
      ...
      desc:
        phantomPassiveRibbon.map((e) => e.label || e.name).join(', ') +
...
function smGetPredictions() {
  ...
  if (lastEl && SMI_RULES[lastEl.name]) {
    SMI_RULES[lastEl.name].forEach((suggested) => {
      if (!currentNames.has(suggested)) {
        suggestions.push({
          name: suggested,
          reason: `Often paired with ${lastEl.label || lastEl.name}`,
          type: 'rule',
        });
      }
    });
    ...
    Object.entries(afterLast)
      ...
      .forEach(([name, count]) => {
        if (!currentNames.has(name) && !suggestions.find((s) => s.name === name)) {
          suggestions.push({
            name,
            reason: `You usually add this after ${lastEl.name} (${count}×)`,
            type: 'learned',
          });
        }
      });
  ...
  if (!hasFOH && els.length >= 4 && !suggestions.find((s) => /foh|speaker/i.test(s.name))) {
    suggestions.push({
      name: 'FOH Speakers',
      reason: 'Add front-of-house output for a complete signal chain',
      type: 'rule',
    });
  }
...
function smRunAnalysis() {
  ...
  try {
    ...
    if (!layoutIssues.length) {
      ...
      if (obs.length) {
        obs.forEach((o) => {
          html += `<div class="smi-ok-state"><span class="smi-ok-icon material-symbols-outlined">check_circle</span><span class="smi-ok-text">${o}</span></div>`;
        });
      } else {
    ...
    results.innerHTML = DOMPurify.sanitize(
      `<div style="padding:16px 0;"><p style="font-family:'Inter';font-size:12px;color:#ff716c;margin:0 0 6px;">Analysis error</p><p style="font-family:'Inter';font-size:11px;color:#484847;margin:0;">${err.message}</p></div>`
    );
  }
}
...
function _smLayoutObservations(els) {
  ...
  const offCanvas = els.filter(
    (e) => e.x < 0 || e.y < 0 || e.x > canvasW + 80 || e.y > canvasH + 80
  );
  ...
  if (mics.length >= 1)
    obs.push(
      `${mics.length} mic${mics.length !== 1 ? 's' : ''} detected — check positions match front-of-stage stands`
    );
...
function smQuickBadge() {
  if (!state.smIntelligenceEnabled) {
    smUpdateBadge(0);
    _smUpdateNavBadge(0);
    return;
  }
  try {
    ...
    /* silent — never block element add */
  }
}
...
function _smUpdateIssueBanner(layoutIssues, conflictIssues) {
  ...
  banner.innerHTML = DOMPurify.sanitize(
    `<span style="font-family:'Manrope',sans-serif;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:${warns.length ? '#ff716c' : '#7aafff'};">${icon} ${label}</span><span style="font-family:'Inter';font-size:10px;color:#484847;margin-left:6px;">Tap to review →</span>`
  );
}
...
function scAutoNumberChannels() {
  ...
  const inputs = state.elements.filter(
    (e) => !/speaker|monitor|wedge|iem|foh|pa\b/i.test(e.name || '')
  );
  inputs.sort((a, b) => priority(a.name) - priority(b.name));
  let ch = 1;
  inputs.forEach((el) => {
    el.channelId = 'CH-' + String(ch++).padStart(2, '0');
  });
  if (typeof renderElements === 'function') renderElements();
  if (typeof markAutosaveDirty === 'function') markAutosaveDirty();
...
function scZoomToFit() {
  ...
  const xs = state.elements.map((e) => e.x),
    ys = state.elements.map((e) => e.y);
  const spread =
    Math.max(Math.max(...xs) - Math.min(...xs), Math.max(...ys) - Math.min(...ys)) + 120;
  const cW = state.canvasW || 800,
    cH = state.canvasH || 500;
...
function _renderStageLayout() {
  ...
  svg.appendChild(
    mk('rect', {
      ...
      height: bH,
      fill: 'none',
      stroke: 'var(--stage-layout-stroke)',
      'stroke-width': '1.2',
  ...
  svg.appendChild(
    mk('line', {
      ...
      x2: cx,
      y2: bY + bH,
      stroke: 'var(--stage-layout-stroke-dim)',
      'stroke-width': '0.8',
...
function _renderCableLabels() {
  ...
  if (!svg) return;
  if (!scCableLengthVisible || !state.connections.length) {
    if (svg.firstChild) svg.innerHTML = DOMPurify.sanitize('');
    return;
  }
  ...
  state.connections.forEach((c) => {
    ...
    bg.setAttribute('fill', 'rgba(14,14,14,0.75)');
    svg.appendChild(bg);
...
function _scMeasureClick(e) {
  ...
  if (!_measure.start) {
    ...
    if (svg) {
      ...
      bg.setAttribute('fill', 'rgba(14,14,14,0.9)');
      bg.setAttribute('stroke', 'rgba(255,116,57,0.5)');
...
function _buildChainView(container) {
  ...
  if (!els.length) {
    container.innerHTML = DOMPurify.sanitize(
      `<p style="font-family:'Inter';font-size:12px;color:#484847;text-align:center;padding:24px 0;">Add elements to the stage<br>to visualize signal chains.</p>`
    );
    return;
  }
  ...
  function traceChain(el, depth) {
    ...
    const chLabel = el.channelId
      ? `<span style="color:#7aafff;font-size:9px;font-weight:700;"> ${el.channelId}</span>`
      : '';
  ...
  container.innerHTML = DOMPurify.sanitize(
    html ||
      `<p style="font-family:'Inter';font-size:12px;color:#484847;text-align:center;padding:24px 0;">No signal chains detected.</p>`
  );
}
...
function _initProfessionalTools() {
  ...
  if (canvas) {
    ...
    if (!document.getElementById('sc-zones-svg')) {
      ...
      z.id = 'sc-zones-svg';
      canvas.insertBefore(z, canvas.firstChild);
    }
    ...
    if (!document.getElementById('sc-measure-svg')) {
      ...
      ms.id = 'sc-measure-svg';
      canvas.appendChild(ms);
    }
  ...
  if (toolbar) {
    const S = (t, bStyle) => {
      ...
      btn.style.cssText =
        'padding:5px 9px;display:flex;align-items:center;gap:4px;border:none;cursor:pointer;font-family:"Manrope",sans-serif;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;background:transparent;color:#767575;transition:all .15s;';
      if (bStyle) Object.assign(btn.style, bStyle);
      btn.innerHTML = DOMPurify.sanitize(t);
      btn.addEventListener('mouseenter', function () {
        if (!this.classList.contains('active')) this.style.color = '#c5ffc9';
      });
      btn.addEventListener('mouseleave', function () {
        if (!this.classList.contains('active')) this.style.color = '#767575';
      });
      return btn;
    };
    const divider = () => {
      ...
      d.style.cssText =
        'width:1px;height:18px;background:rgba(72,72,71,0.4);margin:0 3px;flex-shrink:0;';
      return d;
    };
    const icon = (ic) =>
      `<span class="material-symbols-outlined" style="font-size:16px;">${ic}</span>`;

    // Find the Clear button and insert Chain + Presets + separator before it
    const clearBtn = [...toolbar.querySelectorAll('button')].find(
      (b) => b.onclick && b.onclick.toString().includes('clearStage')
    );

    const bChain = S(`${icon('account_tree')}<span class="desktop-only">Chain</span>`);
    bChain.title = 'Signal Chain Builder';
    bChain.onclick = scShowSignalChain;
  ...
  if (smiSection && !document.getElementById('settings-smart-zoom-row')) {
    ...
    row.innerHTML = DOMPurify.sanitize(
      `<span class="settings-section-hint" style="margin:0;">Smart Zoom on Select</span><button id="settings-smart-zoom-toggle" class="toggle-switch" onclick="scToggleSmartZoom()" title="Auto-focus selected element"></button>`
    );
...
function smComputeReadinessScore() {
  ...
  let completeness = 0;
  if (els.some((e) => /mic|drum|guitar|bass|key|piano|vocal/i.test(e.name || '')))
    completeness += 8;
  if (els.some((e) => /foh|pa speaker|main speaker|monitor|wedge|iem/i.test(e.name || '')))
    completeness += 8;
...
function _applySafetyHighlights(issues) {
  ...
  document
    .querySelectorAll('.sc-safety-hl')
    .forEach((el) => el.classList.remove('sc-safety-hl', 'sc-safety-high', 'sc-safety-medium'));
...
(function () {
  ...
  window.smRunAnalysis = function () {
    try {
      ...
      console.warn('[SC] smRunAnalysis base error:', e);
      return;
    }
    try {
      const results = document.getElementById('sm-intel-results');
      ...
      console.warn('[SC] smRunAnalysis extras error:', e);
    }
  };
})();
...
function _getElPresets() {
  try {
    return JSON.parse(localStorage.getItem(SC_EL_PRESETS_KEY) || '[]');
  } catch {
    return [];
  }
}
function _setElPresets(arr) {
  try {
    localStorage.setItem(SC_EL_PRESETS_KEY, JSON.stringify(arr));
  } catch {}
}
...
function _scPrompt(title, defaultVal, onConfirm) {
  ...
  overlay.style.cssText =
    'position:absolute;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.55);backdrop-filter:blur(2px);';
  const box = document.createElement('div');
  box.style.cssText =
    'background:#111217;border:1px solid rgba(122,175,255,0.2);border-radius:10px;padding:20px 22px 16px;min-width:270px;box-shadow:0 12px 48px rgba(0,0,0,0.8);display:flex;flex-direction:column;gap:12px;';
  const lbl = document.createElement('p');
  lbl.textContent = title;
  lbl.style.cssText =
    'margin:0;font-family:"Manrope",sans-serif;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#c8c8c6;';
  const inp = document.createElement('input');
  inp.type = 'text';
  inp.value = defaultVal || '';
  inp.style.cssText =
    'width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(122,175,255,0.2);border-radius:6px;padding:8px 10px;font-family:"Manrope",sans-serif;font-size:13px;color:#f0f0ee;outline:none;';
  ...
  btnCancel.style.cssText =
    'padding:7px 14px;font-family:"Manrope",sans-serif;font-size:11px;font-weight:600;background:transparent;border:1px solid rgba(255,255,255,0.12);border-radius:6px;color:#888;cursor:pointer;';
  const btnSave = document.createElement('button');
  btnSave.textContent = 'Save';
  btnSave.style.cssText =
    'padding:7px 16px;font-family:"Manrope",sans-serif;font-size:11px;font-weight:700;background:#7aafff;border:none;border-radius:6px;color:#0e0e0e;cursor:pointer;';
  function confirm() {
...
function scSaveAsPreset(nameHint) {
  const selIds =
    typeof multiSel !== 'undefined' && multiSel.size > 0
      ? [...multiSel].map(String)
      : state.selectedId
        ? [String(state.selectedId)]
        : null;
  if (!selIds || !selIds.length) {
    _scToast('Select elements first, then save as preset');
    return;
  }

  const selEls = state.elements.filter((e) => selIds.includes(String(e.id)));
  if (!selEls.length) {
    _scToast('No elements found');
    return;
  }
  ...
  if (nameHint && nameHint.trim()) {
    _doSave(nameHint.trim());
  } else {
    _scPrompt('Name this preset', '', _doSave);
  }
}
...
(function () {
  ...
  if (document.readyState === 'loading')
    document.addEventListener('DOMContentLoaded', _initPresetsHover);
...
(function () {
  ...
  window.pushHistory = function () {
    try {
      ...
      console.warn('[SC] pushHistory base error:', e);
      return;
    }
    try {
...
(function () {
  ...
  window.renderElements = function () {
    try {
      ...
      console.warn('[SC] renderElements chain error:', e);
    }
    try {
      _applyPinBadges();
    } catch (e) {
      console.warn('[SC] _applyPinBadges error:', e);
    }
  };
})();
...
(function () {
  ...
  window.generateShareLink = async function () {
    ...
    try {
      ...
      if (typeof showToast === 'function') showToast('Failed to encode project.');
      return;
    }
...
(function _detectPwShare() {
  ...
  window.addEventListener('DOMContentLoaded', () => _showPwGate(storedHash, encodedPayload), {
    once: true,
  });
...
function _showPwGate(storedHash, encodedPayload) {
  ...
  overlay.style.cssText =
    'position:absolute;inset:0;z-index:99999;background:#0a0a0c;display:flex;align-items:center;justify-content:center;';
  overlay.innerHTML = DOMPurify.sanitize(`
...
async function _verifyPwShare(storedHash, encodedPayload) {
  ...
  try {
    ...
    if (errEl) {
      errEl.textContent = 'Failed to decode link.';
...
(function featuresInit() {
  ...
  setTimeout(() => {
    try {
      ...
      console.warn('[SC] _updateOnlineStatus error:', e);
    }
    try {
      renderLayerPanel();
    } catch (e) {
      console.warn('[SC] renderLayerPanel error:', e);
    }
    try {
      updateConflictBanner();
    } catch (e) {
      console.warn('[SC] updateConflictBanner error:', e);
    }
    try {
      _smRender();
    } catch (e) {
      console.warn('[SC] _smRender error:', e);
    }
    try {
      _initProfessionalTools();
    } catch (e) {
